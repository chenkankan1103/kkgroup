...
def get_user_field(user_id: Union[int, str], field: str, default: Any = None) -> Any:
    """
    獲取用戶特定欄位的值
...
def set_user_field(user_id: Union[int, str], field: str, value: Any) -> bool:
    """
    設置用戶特定欄位的值
...
async def async_set_user_field(
    user_id: Union[int, str], field: str, value: Any
) -> bool:
    """
    ...
    return await loop.run_in_executor(
        None, lambda: set_user_field(user_id, field, value)
    )
...
CENTRAL_RESERVE_DIGITAL_USD_FIELD = (
    "central_reserve_digital_usd"  # 中央數位美金儲備池欄位名
)
