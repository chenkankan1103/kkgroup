...
class SheetSyncManager:
    ...
    def sync_sheet_to_db(
        ...
        add_log(
            "bot",
            f"✅ 同步完成: 新增 {stats['inserted']}, 更新 {stats['updated']}, 錯誤 {stats['errors']}, 總計 {stats['total_parsed']}",
        )
    ...
    def _parse_records(
        ...
        if "user_id" not in headers:
            ...
            if detected_idx is not None:
                add_log(
                    "bot",
                    f"🔎 自動偵測 user_id: 第 {detected_idx+1} 列 ('{headers[detected_idx]}')",
                )
                headers[detected_idx] = "user_id"
            else:
                add_log("bot", "❌ 無法偵測 user_id 欄位，使用第 1 列作為 user_id")
                headers[0] = "user_id"
        ...
        for row_idx, row_values in enumerate(rows, start=1):
            try:
                ...
                if (
                    not record.get("nickname") or record.get("nickname") == ""
                ) and record.get("user_name"):
                ...
                add_log(
                    "bot",
                    f"✓ 行 {row_idx}: user_id={record['user_id']}, nickname={nickname}",
                )
        ...
        for nickname, same_name_recs in nickname_to_records.items():
            if len(same_name_recs) > 1:
                # 有多個同名用戶
                min_record = min(
                    same_name_recs, key=lambda r: int(r.get("user_id", float("inf")))
                )
                ...
                for rec in same_name_recs:
                    if rec["user_id"] != min_record["user_id"]:
                        add_log(
                            "bot",
                            f"👻 過濾幽靈帳號: {nickname} (user_id {rec['user_id']}) → 保留 {min_record['user_id']}",
                        )
                        removed_ghosts += 1
            else:
                # 只有一個
    ...
    def _sync_records_to_db(self, records: List[Dict[str, Any]]) -> Dict[str, int]:
        ...
        for i, record in enumerate(records, 1):
            try:
                ...
                try:
                    user_id = int(user_id)
                    if user_id == 0:
                        error_msg = "user_id 為 0，無效的用戶"
                        add_log("bot", f"⏭️ 記錄 {i}: {error_msg}")
                        stats["errors"] += 1
                        stats["error_details"].append(
                            {"record": i, "reason": error_msg}
                        )
                        continue
                except (ValueError, TypeError) as e:
                    error_msg = (
                        f"user_id 無效或無法轉換: '{record.get('user_id')}' ({str(e)})"
                    )
                ...
                if user_id in seen_user_ids:
                    add_log(
                        "bot",
                        f"⚠️ 記錄 {i}: 在本次同步中有重複的 user_id {user_id} (已跳過)",
                    )
                    stats["duplicates"] += 1
                    continue
                ...
                add_log(
                    "bot",
                    f"✓ 記錄 {i}: [{action}] user_id={user_id}, 欄位數={len(cleaned_record)}",
                )
                try:
        ...
        add_log(
            "bot",
            f"✅ 同步記錄完成: 新增 {stats['inserted']} 個用戶, 更新 {stats['updated']} 個用戶, 重複 {stats['duplicates']} 行, 錯誤 {stats['errors']} 行",
        )
    ...
    def clean_virtual_accounts(self) -> Tuple[int, int]:
        ...
        try:
            ...
            virtual_users = [
                u
                for u in all_users
                if str(u.get("nickname", "")).startswith("Unknown_")
            ]
            ...
            if virtual_users:
                ...
                for user in virtual_users[:5]:
                    add_log(
                        "bot", f"   - {user.get('user_id')}: {user.get('nickname')}"
                    )
