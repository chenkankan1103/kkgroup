...
class SheetDrivenDB:
    ...
    def _infer_sql_type(self, header: str) -> str:
        ...
        # 時間戳欄位
        if any(word in header_lower for word in ["date", "time", "timestamp", "at"]):
            return "TEXT DEFAULT NULL"

        # JSON 欄位 (用於複雜結構)
        if any(
            word in header_lower
            for word in ["config", "setting", "data", "json", "info", "inventory"]
        ):
            return "TEXT DEFAULT '{}'"
    ...
    def update_user_field(
        ...
        if isinstance(current, str):
            try:
                ...
                elif current.isdigit() or (
                    current.startswith("-") and current[1:].isdigit()
                ):
                    current = int(current)
                else:
        ...
        for i, row in enumerate(rows):
            try:
                ...
                if user_id in seen_user_ids:
                    print(
                        f"⚠️ 第 {i+2} 行: 在本次同步中有重複的 user_id {user_id} (已跳過)"
                    )
                    stats["duplicates"] += 1
                    continue
    ...
    def export_to_sheet_format_ordered(
        self, desired_headers: List[str]
        ...
        for db_row in cursor.fetchall():
            ...
            if (
                not row_dict.get("nickname") or row_dict.get("nickname") == ""
            ) and row_dict.get("user_name"):
        ...
        if any(word in header_lower for word in ["date", "time", "recovery"]):
            if isinstance(value, str):
                ...
                try:
                    ...
                    # 或者嘗試從 Unix 時間戳解析
                    if (
                        str(value).startswith("Wed ")
                        or str(value).startswith("Thu ")
                        or str(value).startswith("Fri ")
                    ):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        for field in ["level", "xp", "kkcoin", "hp", "stamina"]:
            if f'"{field}"' in columns or field in columns:
                try:
                    cursor.execute(
                        f'SELECT AVG("{field}"), MAX("{field}"), MIN("{field}") FROM {self.table_name}'
                    )
