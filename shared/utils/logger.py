...
# ✅ 根據不同 bot 設定不同的啟動延遲
BOT_DELAYS = {"BOT": 5, "SHOPBOT": 8, "UIBOT": 11}
...
def send_startup_messages():
    ...
    normal = [
        line for line in buffer_copy if line not in errors and line not in success
    ]
    ...
    if success:
        ...
        # 只顯示關鍵成功訊息（模組數量等）
        key_success = [
            s for s in success if "擴展" in s or "指令" in s or "已就緒" in s
        ]
