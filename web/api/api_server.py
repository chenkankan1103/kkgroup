...
def get_stats():
    ...
    try:
        ...
        active_traders = len(
            [u for u in all_users if float(u.get("kkcoin", 0) or 0) > 0]
        )
...
if __name__ == "__main__":
    ...
    print(f"\n{'='*60}")
    print("🚀 KKCoin API 伺服器啟動")
    print(f"{'='*60}")
    print(f"📍 位置: {host}:{port}")
    print(f"🔧 Debug: {debug_mode}")
    print("📡 可用端點:")
    print("   - GET  /api/stats          (即時統計)")
    print("   - GET  /api/stats/detailed (詳細統計)")
    print("   - GET  /api/config         (前端配置)")
