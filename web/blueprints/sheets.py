...
def api_sync_sheet():
    ...
    try:
        ...
        if not headers:
            return jsonify({"status": "error", "message": "表頭不能為空"}), 400
        ...
        logger.info(
            f"✅ 同步完成: 新增 {sync_stats['inserted']}, 更新 {sync_stats['updated']}, 錯誤 {sync_stats['errors']}, 重複 {sync_stats['duplicates']}"
        )
        ...
        result = {
            ...
            "error_details": sync_stats.get("error_details", [])[
                :20
            ],  # 只返回前 20 個錯誤
            "timestamp": datetime.now().isoformat(),
        }
...
def api_export_db():
    ...
    try:
        ...
        if request.method == "POST":
            ...
            if sheet_headers:
                logger.info(
                    f"📋 使用提供的 SHEET 表頭順序（{len(sheet_headers)} 欄位）"
                )
        ...
        if sheet_headers:
            ...
            print("✅ 按 SHEET 表頭順序導出完成")
        else:
            # ❌ 舊方式：按 DB 順序導出（向後相容）
            logger.warning(
                "⚠️ 未提供 SHEET 表頭，使用 DB 順序導出（建議提供表頭以保持對齊）"
            )
            headers, rows = db.export_to_sheet_format()
            print("⚠️ 按 DB 順序導出（需要提供 SHEET 表頭以防止欄位錯位）")
...
def api_get_user(user_id):
    ...
    if not _sheet_check_member_or_key():
        return jsonify({"status": "error", "message": "未認證，請先 Discord 登入"}), 401
    ...
    try:
        ...
        if old_user:
            for key, new_val in data.items():
                ...
                if old_val != new_val:
                    logger.info(
                        f"📝 [ADMIN] user_id={user_id}: {key} {old_val} → {new_val}"
                    )
