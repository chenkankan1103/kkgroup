...
if TEMP_VC_TRIGGER_CHANNEL_ID == 0:
    print(
        "⚠️ 環境變數 TEMP_VC_CATEGORY_ID 未設定，請在 .env 或系統環境中指定入口語音頻道的 ID"
    )
else:
    print(
        f"✅ 已讀取 TEMP_VC_CATEGORY_ID = {TEMP_VC_TRIGGER_CHANNEL_ID} (trigger channel)"
    )
...
def _get_db_connection():
    """獲取 aiosqlite 連線物件（非 async 函數，直接返回連線物件供 async with 使用）"""
    conn = aiosqlite.connect(DB_PATH)
    # WAL 模式設定將在連線建立時自動套用（透過 __aenter__）
    return conn


async def _configure_db_connection(conn):
    """配置資料庫連線設定（在 async with 進入後呼叫）"""
...
AI_API_KEY = os.getenv(
    "AI_API_KEY", "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"
)
...
class ScamHub(commands.Cog):
    ...
    async def _load_active_rooms(self):
        ...
        try:
            ...
            for row in rows:
                ...
                try:
                    last_active = (
                        datetime.fromisoformat(last_active_str)
                        if last_active_str
                ...
                try:
                    next_event_time = (
                        datetime.fromisoformat(next_event_str)
                        if next_event_str
                        else datetime.utcnow() + timedelta(minutes=random.randint(1, 3))
                    )
                except (ValueError, TypeError):
                    next_event_time = datetime.utcnow() + timedelta(
                ...
                if members:
                    ...
                    print(
                        f"[ScamHub] 啟動檢查: 頻道 {room_name} 有 {len(members)} 人，恢復正常運作"
                    )
                else:
    ...
    async def _get_voice_members(self, vc):
        ...
        try:
            ...
            print(
                f"[ScamHub] 🔍 _get_voice_members: 頻道 {vc.name} (ID: {vc.id}) 成員: {[m.name for m in members]}"
            )
            return members
        except Exception as e:
    ...
    async def _update_room_db(
        self, room_id: int, next_event_time: datetime = None, message_id: int = None
    ):
        """更新數據庫中的房間狀態"""
        try:
            async with _get_db_connection() as db:
    ...
    async def _delete_room_from_db(self, room_id: int):
        ...
        try:
            async with _get_db_connection() as db:
                await _configure_db_connection(db)
                await db.execute(
                    "UPDATE scam_rooms SET is_active=0 WHERE room_id=?", (room_id,)
                )
                await db.commit()
        except Exception as e:
    ...
    async def _schedule_deletion(self, room_id: int):
        ...
        if room_id not in self.active_rooms:
            print(
                f"[ScamHub] ⚠️ _schedule_deletion: 房間 {room_id} 不在 active_rooms 中，無法啟動刪除倒數"
            )
            return
        ...
        async def deletion_countdown():
            ...
            try:
                ...
                print(
                    f"[ScamHub] 🕐 房間 {room_name} (ID: {room_id}) 無人，5 分鐘後將自動刪除 - 倒數計時已啟動"
                )
                await asyncio.sleep(INACTIVE_TIMEOUT)  # 等待 5 分鐘

                elapsed = (datetime.utcnow() - start_time).total_seconds()
                print(
                    f"[ScamHub] 🕐 倒數計時結束 (耗時 {elapsed:.1f} 秒)，檢查房間 {room_id} 是否仍無人"
                )

                # 5 分鐘後檢查是否還是無人
                if room_id not in self.active_rooms:
                    print(
                        f"[ScamHub] ⚠️ 房間 {room_id} 已從 active_rooms 中移除，停止刪除程序"
                    )
                    return
                ...
                print(
                    f"[ScamHub] 🔍 檢查結果: 頻道 {room_name} 成員數: {member_count}, 成員列表: {[m.name for m in members]}"
                )

                if member_count == 0:
                    print(
                        f"[ScamHub] 🗑️ 執行刪除: {vc.name} (ID: {room_id}) - 無人語音頻道 (成員數: 0)"
                    )
                    try:
                        await vc.delete(reason="自動刪除無人語音頻道 (5分鐘)")
                        print(
                            f"[ScamHub] ✅ 成功刪除無人頻道 {vc.name} (ID: {room_id})"
                        )
                    except discord.Forbidden:
                        print(
                            f"[ScamHub] ❌ 權限不足：無法刪除頻道 {room_id} (需要 manage_channels 權限)"
                        )
                    except discord.HTTPException as e:
                        print(f"[ScamHub] ❌ HTTP 錯誤：刪除頻道 {room_id} 失敗 - {e}")
                    except Exception as e:
                        print(
                            f"[ScamHub] ❌ 刪除頻道 {room_id} 時發生未預期錯誤: {type(e).__name__}: {e}"
                        )
                    finally:
                        # 確保清理所有相關數據
                        self.active_rooms.pop(room_id, None)
                    ...
                    print(
                        f"[ScamHub] ⏸️ 倒數計時完成但檢查到 {member_count} 人在頻道中 {[m.name for m in members]}，取消刪除"
                    )
                ...
                print(
                    f"[ScamHub] ❌ 刪除房間 {room_id} 時發生未預期的錯誤: {type(e).__name__}: {e}"
                )
                import traceback
            ...
            async with aiohttp.ClientSession() as session:
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        return "詐騙團隊執行了一場經典的電話詐騙行動，冒充銀行客服誘騙受害者透露個人資料。"
        except Exception as e:
            print(f"生成詐騙事件時發生錯誤: {e}")
            return "詐騙團隊進行了一次成功的網路釣魚攻擊，透過假冒的購物網站騙取了信用卡資訊。"
    ...
    async def setup_scam_hub(self, interaction: discord.Interaction):
        ...
        await interaction.response.send_message(
            f"✅ 語音入口已建立：{channel.mention}", ephemeral=True
        )
    ...
    async def on_voice_state_update(self, member, before, after):
        # Debug: 印出當前頻道和 ID
        print(
            f"[ScamHub] on_voice_state_update triggered: member={member} ({member.id}), before={getattr(before.channel, 'id', None)}, after={getattr(after.channel, 'id', None)}"
        )
        if after.channel:
            print(
                f"voice_state_update: member={member.display_name}, after_channel={after.channel.name}, id={after.channel.id}"
            )
        else:
            print(
                f"voice_state_update: member={member.display_name}, after_channel=None"
            )
        ...
        if after.channel and after.channel.id == TEMP_VC_TRIGGER_CHANNEL_ID:
            ...
            if member_role:
                await new_channel.set_permissions(
                    member_role, view_channel=True, connect=True
                )
                print(
                    f"設定頻道 {new_channel.name} 為私人頻道，僅 {member_role.name} 身分組可見"
                )
            else:
            ...
            await self._save_room_to_db(
                new_channel.id, guild.id, member.id, new_channel.name, next_event_time
            )

            print(
                f"創建新的詐騙小組頻道: {new_channel.name} (ID: {new_channel.id}), 房主: {member.display_name}"
            )
            ...
            try:
                await new_channel.send(
                    content=member.mention, embed=embed, view=RoomControlView()
                )
            except Exception as e:
        ...
        if after.channel and after.channel.id in self.active_rooms:
            ...
            # ✅ 新邏輯：有人加入時取消倒數計時
            print(
                f"[ScamHub] 👥 用戶 {member.display_name} 加入頻道 {after.channel.name}, 目前人數: {current_members}"
            )
        ...
        if before.channel and before.channel.id in self.active_rooms:
            ...
            print(
                f"[ScamHub] 👤 用戶 {member.display_name} 離開頻道 {before.channel.name}, 剩餘人數: {current_members}"
            )
            ...
            if current_members > 0:
                ...
                # ✅ 頻道空了，啟動 5 分鐘倒數計時
                print(
                    f"[ScamHub] ⚡ 頻道無人! 調用 _schedule_deletion({before.channel.id})"
                )
                await self._schedule_deletion(before.channel.id)
                print(
                    f"[ScamHub] ✏️ 頻道 {before.channel.name} (ID: {before.channel.id}) 已空，倒數計時已啟動"
                )
