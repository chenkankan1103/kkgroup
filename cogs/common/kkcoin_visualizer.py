...
async def create_colorful_leaderboard_image(members_data, limit=20):
    ...
    try:
        ...
        FONT_BIG = FONT_RANK = FONT_NAME = FONT_KKCOIN = FONT_SMALL = FONT_DESC = (
        ...
        for i, (member, kkcoin) in enumerate(members_data):
            ...
            if is_top3:
                ...
                if rank == 1:
                    ...
                    draw.rectangle(
                        [(MARGIN, y), (WIDTH - MARGIN, y + 75)], fill=bg_fill
                    )
                    border_color = (
                        (192, 192, 192) if rank == 2 else (205, 127, 50)
                    )  # 銀/銅
            else:
                # 其他排名背景
                draw.rectangle(
                    [(MARGIN, y), (WIDTH - MARGIN, y + 75)], fill=row_bg_color
                )
                border_color = None
            ...
            try:
                ...
                if avatar_url:
                    async with session.get(
                        ...
                        if resp.status == 200:
                            avatar = Image.open(io.BytesIO(await resp.read())).convert(
                                "RGBA"
                            )
                            ...
                            if is_top3:
                                ...
                                halo = Image.new(
                                    "RGBA",
                                    (avatar_size + 8, avatar_size + 8),
                                    halo_color,
                                )
                                halo_mask = Image.new(
                                    "L", (avatar_size + 8, avatar_size + 8), 0
                                )
                                halo_mask_draw = ImageDraw.Draw(halo_mask)
                                halo_mask_draw.ellipse(
                                    [(0, 0), (avatar_size + 8, avatar_size + 8)],
                                    fill=255,
                                )
            ...
            draw.text(
                (name_x, y + 32), kkcoin_text, fill=kkcoin_color, font=FONT_KKCOIN
            )
    ...
    draw.line(
        [(MARGIN, desc_y), (WIDTH - MARGIN, desc_y)], fill=(180, 190, 210), width=2
    )

    draw.text(
        (MARGIN + 10, desc_y + 15), "💡 KK幣獲得方法", fill=(30, 30, 80), font=FONT_RANK
    )
    descriptions = [
...
async def create_chart_image(members_data, chart_type="bar", limit=10):
    ...
    if not MATPLOTLIB_AVAILABLE:
        raise RuntimeError(
            "matplotlib 未安裝，無法生成圖表。請執行: pip install matplotlib numpy"
        )
    ...
    if chart_type == "bar":
        ...
        ax.set_title(
            f"KK幣分布 - 圓餅圖（前{limit}名）", fontsize=16, fontweight="bold", pad=20
        )
...
async def create_weekly_stats_image(
    total_coins, this_week_total, last_week_total, member_count
):
    """
    創建本週統計圖表
    ...
    fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(
        2, 2, figsize=(14, 10), facecolor="#f5f8fc"
    )
    ...
    bars = ax1.bar(
        weeks, totals, color=colors_comp, edgecolor="black", linewidth=2, width=0.6
    )
    ...
    # 2. 園區總資產變化（圖表）
    wealth_data = [("館長", 15000), ("夜店", 12000), ("競技場", 10000), ("其他", 8000)]
    ...
    ax4.text(
        0.5, 0.35, "名", fontsize=11, ha="center", transform=ax4.transAxes, color="#888"
    )

    # 背景色
    rect_member = plt.Rectangle(
        (0.05, 0.05),
        0.9,
        0.9,
        transform=ax4.transAxes,
...
async def create_weekly_mvp_image(
    mvp_member, this_week_coins, rank_position, total_members
):
    """
    創建本週績效王卡片
                ...
                async with session.get(
                    ...
                    if resp.status == 200:
                        ...
                        circle_img = Image.new(
                            "RGBA", (avatar_size + 20, avatar_size + 20), (0, 0, 0, 0)
                        )
                        circle_draw = ImageDraw.Draw(circle_img)
                        circle_draw.ellipse(
                            [(0, 0), (avatar_size + 20, avatar_size + 20)],
                            outline=(255, 215, 0),
                            width=4,
                        )
                        ...
                        img.paste(
                            circle_img, (avatar_x - 10, avatar_y - 10), circle_img
                        )
    ...
    # 本週新增KK幣
    draw.text(
        (info_x, avatar_y + 110), "本週新增", fill=(200, 200, 200), font=FONT_LABEL
    )
    draw.text(
        (info_x, avatar_y + 145),
        f"{int(this_week_coins):,}",
        fill=(100, 255, 150),
        font=FONT_STATS,
    )
    draw.text(
        (info_x + 350, avatar_y + 150), "KK幣", fill=(100, 255, 150), font=FONT_LABEL
    )

    # 排行位置
    draw.text(
        (info_x, avatar_y + 220),
        f"總排行：第 {rank_position} 名（共 {total_members} 人）",
        fill=(150, 200, 255),
        font=FONT_DESC,
    )
    ...
    time_str = datetime.now().strftime("%Y-%m-%d %H:%M")
    draw.text(
        (50, HEIGHT - 40), f"更新時間：{time_str}", fill=(150, 150, 150), font=FONT_DESC
    )
...
async def create_comprehensive_dashboard(
    members_data, limit=10, total_coins=None, this_week_total=None, last_week_total=None
):
    """
    創建綜合仪表板 - 長條圖 + 圓餅圖 + 周統計
    ...
    ax2.set_title("🍰 KK幣分布圖", fontsize=13, fontweight="bold")
    ...
    colors_comp = ["#FF6464", "#6496FF"]
    bars = ax_compare.bar(
        weeks, totals, color=colors_comp, edgecolor="black", linewidth=1.5, width=0.5
    )
    ...
    growth_text = (
        f"增長 +{growth_rate:.1f}%" if growth_rate > 0 else f"下降 {growth_rate:.1f}%"
    )
    ...
    rect1 = plt.Rectangle(
        ...
        1,
        1,
        transform=ax_metrics1.transAxes,
        facecolor="#fff9e6",
        edgecolor="#FFD700",
        linewidth=2,
        zorder=-1,
    ...
    rect2 = plt.Rectangle(
        ...
        1,
        1,
        transform=ax_metrics2.transAxes,
        facecolor="#e6f2ff",
        edgecolor="#6496FF",
        linewidth=2,
        zorder=-1,
    ...
    rect3 = plt.Rectangle(
        ...
        1,
        1,
        transform=ax_metrics3.transAxes,
        facecolor="#e6ffe6",
        edgecolor="#6BBF59",
        linewidth=2,
        zorder=-1,
