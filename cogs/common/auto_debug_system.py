...
class AutoDebugSystem:
    def __init__(self):
        ...
        self.github_mode = (
            os.getenv("AUTO_DEBUG_GITHUB_MODE", "escalate").strip().lower()
        )
        self.escalate_severity = (
            os.getenv("AUTO_DEBUG_ESCALATE_SEVERITY", "critical").strip().lower()
        )
    ...
    async def analyze_locally(self, error_data: Dict) -> Dict:
        ...
        try:
            ...
            logger.warning(
                f"本地 AI 分析失敗，將使用 fallback 摘要: {type(exc).__name__}: {exc}"
            )
            fallback["impact"] = str(exc)

        return fallback

    def should_escalate_to_github(
        self, error_data: Dict, local_analysis: Optional[Dict]
    ) -> bool:
        ...
        severity = str(error_data.get("severity") or "unknown").lower()
        analysis_status = str(
            (local_analysis or {}).get("analysis_status") or ""
        ).lower()
    ...
    def save_local_artifact(
        self, error_data: Dict, local_analysis: Dict, escalated: bool
    ) -> str:
        """將本地分析結果寫入 archive，方便 VM 上直接追查。"""
        incident_signature = _build_incident_signature(
            error_data.get("errors") or {}, error_data.get("severity", "unknown")
        )
    ...
    def _build_detection_message(
        self,
        result: Dict,
        local_analysis: Optional[Dict] = None,
        artifact_path: str = "",
    ) -> str:
        lines = []
            ...
            for service in _MONITORED_SERVICES:
                ...
                if action == "local-heal":
                    ...
                    if heal_result.get("success"):
                        ...
                        severity_rank = (
                            "medium" if severity_rank == "low" else severity_rank
                        )
                        continue
    ...
    async def _handle_detection_result(self, result: Dict) -> int:
        ...
        artifact_path = self.save_local_artifact(
            result, local_analysis, should_escalate
        )
        status_text = "已完成本地分析"
        if should_escalate:
        ...
        await self.send_notification(
            self._build_detection_message(
                result, local_analysis=local_analysis, artifact_path=artifact_path
            ),
            result["severity"],
        ...
        try:
            ...
            payload = {
                ...
                "client_payload": {
                    ...
                    "incident_signature": _build_incident_signature(
                        error_data["errors"], error_data["severity"]
                    ),
                },
            }
            ...
            async with aiohttp.ClientSession(timeout=self.http_timeout) as session:
                async with session.post(url, json=payload, headers=headers) as response:
                    if response.status == 204:
                        ...
                        logger.error(
                            f"❌ 觸發 GitHub Actions 失敗: {response.status} - {error_text}"
                        )
                        return False
        ...
        try:
            async with aiohttp.ClientSession(timeout=self.http_timeout) as session:
                async with session.post(
                    self.webhook_url, json=webhook_data
                ) as response:
                    if response.status == 204:
...
async def main():
    ...
    if not debug_system.github_token:
        logger.info(
            "未設置 GitHub token，auto debug 將以本地分析模式運行，不升級至 GitHub Actions"
        )
    run_once = os.getenv("AUTO_DEBUG_RUN_ONCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
