...
class GoogleSheetsSync(commands.Cog):
    ...
    def _init_gspread(self):
        ...
        try:
            ...
            print(
                f"📑 打開試算表，工作表清單: {[ws.title for ws in spreadsheet.worksheets()]}"
            )
            ...
            print(
                f"   - 行數: {len(all_values)}, 列數: {len(all_values[0]) if all_values else 0}"
            )
    ...
    async def auto_sync_loop(self):
        ...
        try:
            ...
            if current_hash != self.last_sheet_hash:
                ...
                print("🔍 [1分鐘檢查] 檢測到 SHEET 內容變化，準備同步...")
                updated, inserted, errors = await self._sync_from_sheet_internal()
                print(
                    f"✅ [1分鐘同步] Google Sheet → 資料庫 (更新: {updated}, 新增: {inserted}, 錯誤: {errors}) ({datetime.now().strftime('%H:%M:%S')})"
                )
            else:
                # Debug: 顯示一下目前的 hash 狀態
                print(
                    f"⏸️ [1分鐘檢查] SHEET 內容未變化，無需同步 ({datetime.now().strftime('%H:%M:%S')})"
                )
    ...
    async def auto_export_loop(self):
        ...
        try:
            ...
            all_records_after = await loop.run_in_executor(
                None, self.sheet.get_all_records
            )
            self.last_sheet_hash = hashlib.md5(
                str(all_records_after).encode()
            ).hexdigest()

            print(
                f"✅ [5分鐘更新] 資料庫 → Google Sheet ({datetime.now().strftime('%H:%M:%S')})"
            )
    ...
    async def _sync_from_sheet_internal(self):
        ...
        try:
            ...
            try:
                ...
                print(
                    f"📋 SHEET 表頭 (第 2 行，共 {len(headers)} 列): {headers[:5]}..."
                )

                # 2. 自動同步 DB schema（添加缺失的欄位）
                print("🔧 檢查並自動同步 DB schema...")
                ...
                for i in range(min(3, len(records))):
                    ...
                    print(
                        f"🔍 記錄 {i+1}: user_id={user_id}, kkcoin={kkcoin}, level={level}"
                    )
                ...
                print(
                    f"✅ [SHEET→DB 同步] 更新={updated}, 新增={inserted}, 錯誤={errors} ({datetime.now().strftime('%H:%M:%S')})"
                )
    ...
    async def _export_to_sheet_internal(self):
        ...
        try:
            ...
            def do_export():
                ...
                # 按 kkcoin DESC 排序
                users_sorted = sorted(
                    users, key=lambda u: u.get("kkcoin", 0), reverse=True
                )
                ...
                for user in users_sorted:
                    ...
                    nickname = (
                        bot_user.display_name if bot_user else f"Unknown_{user_id}"
                    )
                    ...
                    data.append(
                        [
                            ...
                            int(user.get("xp", 0)),
                            int(user.get("kkcoin", 0)),
                            user.get("title", "新手"),
                            int(user.get("hp", 100)),
                            int(user.get("stamina", 100)),
                            user.get("inventory", ""),
                            user.get("character_config", ""),
                            int(user.get("face", 20000)),
                            int(user.get("hair", 30000)),
                            int(user.get("skin", 12000)),
                            int(user.get("top", 1040010)),
                            int(user.get("bottom", 1060096)),
                            int(user.get("shoes", 1072288)),
                            int(user.get("streak", 0)),
                            user.get("last_work_date", "") or "",
                            user.get("last_action_date", "") or "",
                            user.get("actions_used", 0),
                            user.get("gender", ""),
                ...
                print("✅ 導出完成: 保留 Row 1（分組標題），更新 Row 2+（表頭和數據）")
                print("   - Row 1: 分組標題（保留）")
                print("   - Row 2: 實際表頭")
