...
class NicknameIDManager(commands.Cog):
    ...
    @app_commands.command(
        name="assign_nickname_id", description="為所有成員設定園編編號"
    )
    ...
    async def assign_nickname_id(self, interaction: discord.Interaction):
        ...
        for member in interaction.guild.members:
            ...
            try:
                ...
                park_id = (
                    f"No.{int(member.id) % 100000:05d}"  # 園編格式：No.00001~99999
                )
                new_nick = (
                    f"{park_id} {base_name}"
                    if not base_name.startswith(park_id)
                    else base_name
                )
    ...
    # 為所有成員移除園編
    @app_commands.command(
        name="remove_nickname_id", description="移除園編編號，還原所有成員原始暱稱"
    )
    @app_commands.checks.has_permissions(manage_nicknames=True)
    ...
    # 單人設定園編
    @app_commands.command(
        name="test_assign_nickname_id", description="為指定成員設定園編編號（單人測試）"
    )
    @app_commands.describe(member="要設定園編的成員")
    @app_commands.checks.has_permissions(manage_nicknames=True)
    async def test_assign_nickname_id(
        ...
        try:
            ...
            await interaction.followup.send(
                f"✅ 成功為 {member.mention} 設定園編編號：`{new_nick}`"
            )
        except Exception as e:
            print(f"❌ 無法變更 {member.display_name} 的暱稱: {e}")
    ...
    # 單人移除園編
    @app_commands.command(
        name="test_remove_nickname_id", description="移除指定成員的園編編號（單人測試）"
    )
    @app_commands.describe(member="要還原暱稱的成員")
    @app_commands.checks.has_permissions(manage_nicknames=True)
    async def test_remove_nickname_id(
        self, interaction: discord.Interaction, member: discord.Member
    ):
        await interaction.response.defer(thinking=True)
        if member.bot or member.nick is None:  # 跳過 BOT 和無暱稱成員
        ...
        try:
            ...
            await interaction.followup.send(
                f"🧼 已成功移除 {member.mention} 的園編編號，恢復為：`{original_nick}`"
            )
        except Exception as e:
            print(f"❌ 無法變更 {member.display_name} 的暱稱: {e}")
