...
class CheckInButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not user:
                error_msg = (
                    f"❌ 打卡失敗: 無法獲取用戶資料 (user: {user_name}, ID: {user_id})"
                )
                logger.error(error_msg)
                await interaction.followup.send(
                    "❌ 無法獲取用戶資料，請稍後重試。", ephemeral=True
                )
                return

            logger.info(
                f"✓ 用戶資料已取得: Lv.{user.get('level')} {user.get('title')} (user: {user_name})"
            )
            ...
            (
                ...
                interaction.user.id, interaction.user, interaction.guild
            )
            ...
            if embeds_tuple and updated_user:
                ...
                logger.info(
                    f"   薪資: {actual_salary:,} / {base_salary:,} KK幣 ({salary_percent}%)"
                )
                ...
                add_log(
                    bot_type,
                    f"✅ {user_name} 打卡成功 - 獲得 {actual_salary:,} KK幣 ({performance})",
                )
                ...
                await interaction.followup.send(
                    "❌ 處理打卡失敗，請稍後再試或聯絡管理員", ephemeral=True
                )
            ...
            if responded:
                try:
                    await interaction.followup.send(
                        "❌ 發生錯誤，請稍後再試或聯絡管理員。", ephemeral=True
                    )
                except:
...
class RestButton(discord.ui.Button):
    def __init__(self):
        super().__init__(
            label="休息一天", style=discord.ButtonStyle.secondary, custom_id="work:rest"
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not user:
                error_msg = (
                    f"❌ 休息失敗: 無法獲取用戶資料 (user: {user_name}, ID: {user_id})"
                )
                logger.error(error_msg)
                await interaction.followup.send(
                    "❌ 無法獲取用戶資料，請稍後重試。", ephemeral=True
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if str(user_id) != user_id_check:
                logger.warning(
                    f"❌ 工作行動失敗: 不是該按鈕的擁有者 (user: {user_name})"
                )
                await interaction.followup.send(
                    "你不能使用別人的工作按鈕！", ephemeral=True
                )
                return
            ...
            logger.info(
                f"✓ 使用者資料: Lv.{current_user.get('level')} {current_user.get('title')} (user: {user_name})"
            )
            ...
            if last_work_date not in [today, yesterday]:
                logger.warning(
                    f"❌ 工作行動失敗: 按鈕已過期 (user: {user_name}, last_work_date: {last_work_date})"
                )
                await interaction.followup.send(
                    "⚠️ 此工作按鈕已過期，請重新打卡領取今日任務！", ephemeral=True
                )
                return

            logger.info(f"💼 處理工作行動: {action} (user: {user_name})")
            embeds_tuple, updated_user, message = await process_work_action(
                user_id, interaction.user, action
            )
            ...
            if embeds_tuple and updated_user:
                ...
                if message and "已經執行過" in message:
                    logger.info(
                        f"ℹ️ 用戶嘗試重複執行行動: {message} (user: {user_name})"
                    )
                else:
                    logger.warning(
                        f"⚠️ 工作行動返回 None: {message or '無訊息'} (user: {user_name})"
                    )
...
class WorkCog(commands.Cog):
    ...
    async def deploy_work_system_when_ready(self):
        ...
        try:
            ...
            logger.info(
                f"🤖 機器人已準備好，開始檢查工作頻道 ID: {self.work_channel_id}"
            )
            ...
            for user in all_users:
                ...
                if last_work_date in [today, yesterday]:
                    try:
                        ...
                        logger.warning(
                            f"⚠️ 註冊用戶 {user.get('user_id')} 的 View 失敗: {e}"
                        )
                        continue
    ...
    async def deploy_work_system(self):
        ...
        try:
            ...
            if not channel:
                ...
                logger.error("   請檢查環境變數 WORK_CHANNEL_ID 是否正確設定")
            ...
            async for message in channel.history(limit=50):
                if message.author == self.bot.user and message.embeds:
                    for embed in message.embeds:
                        if embed.title and (
                            "KK園區值勤系統" in embed.title or "詐騙園區" in embed.title
                        ):
            ...
            logger.error("❌ 發送訊息時發生 HTTP 錯誤", exc_info=True)
        except Exception:
    ...
    def create_work_system_embed(self):
        ...
        embed.add_field(name="💼 職位階級表", value=salary_info, inline=False)
    ...
    @discord.app_commands.command(
        name="work_info", description="查看詳細的工作行動資訊"
    )
        ...
        try:
            ...
            if level < 5:
                ...
                if can_level_up:
                    ...
                    current_days_text = (
                        f"{info['current_days']}/{info['required_days']} 天"
                    )
                    weeks_text = f"({current_weeks}/{required_weeks} 周)"
                    xp_text = f"{info['current_xp']:,}/{info['required_xp']:,} XP"
                    days_met_text = (
                        "✅ 出勤達標！" if info["days_met"] else "⏳ 還需連續出勤"
                    )
                    xp_met_text = (
                        "✅ 經驗達標！" if info["xp_met"] else "⏳ 還需累積經驗"
                    )
            ...
            try:
                if isinstance(actions_used_raw, dict):
                    ...
                    actions_used = (
                        json.loads(actions_used_raw) if actions_used_raw else {}
                    )
                else:
                    actions_used = {}
            except (json.JSONDecodeError, TypeError):
            ...
            embed.add_field(name="🎯 今日行動紀錄", value=actions_status, inline=False)
    ...
    async def work_health(self, interaction: discord.Interaction):
        try:
            ...
            active_yesterday = sum(
                1 for u in all_users if u.get("last_work_date") == yesterday
            )
            ...
            try:
                ...
                work_action_views = [
                    v for v in all_views if isinstance(v, WorkActionView)
                ]
                view_count = len(work_action_views)
            except:
                view_count = "無法取得"

            embed = discord.Embed(title="🏥 工作系統健康檢查", color=0x00FF00)
            ...
            embed.add_field(name="📊 用戶統計", value=user_stats, inline=True)
            ...
            embed.add_field(name="🔧 系統狀態", value=system_stats, inline=True)
            ...
            if warnings:
                warnings_text = "\n".join(warnings)
                embed.add_field(name="⚠️ 警告", value=warnings_text, inline=False)
            ...
            traceback.print_exc()
            await interaction.followup.send(
                f"❌ 健康檢查失敗: {str(e)}", ephemeral=True
            )

    @discord.app_commands.command(
        name="work_rebuild",
        description="重建工作系統 View（管理員專用，用於修復按鈕失效）",
    )
    async def work_rebuild(self, interaction: discord.Interaction):
        try:
            ...
            active_count = sum(
                1 for u in all_users if u.get("last_work_date") in active_dates
            )

            rebuild_stats = f"已為 **{active_count}** 位近期活躍用戶重建 View"
            embed.add_field(name="📊 重建統計", value=rebuild_stats, inline=False)
    ...
    @discord.app_commands.command(
        name="work_deploy", description="手動部署工作系統（管理員專用）"
    )
