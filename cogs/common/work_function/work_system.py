    ...
    try:
        ...
        if level < 6:
            ...
            if can_level_up:
                ...
                days_progress = create_progress_bar(
                    info["current_days"], info["required_days"]
                )
                xp_progress = create_progress_bar(
                    info["current_xp"], info["required_xp"]
                )
        ...
        embed.add_field(name="🎯 可執行行動", value=actions_text, inline=False)
        ...
        return discord.Embed(
            title="⚠️ 資訊顯示錯誤", description="無法載入完整資訊，請聯絡管理員"
        )
...
def create_level_up_embed(user_obj, old_level, new_level, bonus_coins):
    ...
    if new_actions:
        ...
        embed.add_field(name="🆕 新解鎖行動", value=new_actions_text, inline=False)
...
async def assign_role(guild, user, new_level):
    ...
    try:
        ...
        all_level_roles = [
            LEVELS[lvl]["role_id"] for lvl in LEVELS.keys() if LEVELS[lvl]["role_id"]
        ]
        ...
        if new_role_id:
            ...
            if new_role:
                await member.add_roles(
                    new_role, reason=f"升級至 {LEVELS[new_level]['title']}"
                )
...
async def process_checkin(user_id, user_obj, guild):
    ...
    try:
        ...
        print(
            f"  📊 初始數據 - Lv.{level}, XP: {current_xp}, 金幣: {current_kkcoin}, 連勤: {streak-1}"
        )
        ...
        print(
            f"  📈 XP 獲得: {xp_gain} (基數: {xp_gain//int(xp_boost) if xp_boost else xp_gain}, 倍率: {xp_boost})"
        )
        ...
        print(
            f"  💰 薪資計算: {int(salary_multiplier*100)}% × {base_salary} × {salary_boost} = {kkcoin_gain} KK幣"
        )
        ...
        if can_level_up and level < 6:
            ...
            leveled_up = True
            print(
                f"  🎊 等級提升! Lv.{old_level} → Lv.{level}, 獲得升級獎勵: {bonus_coins} KK幣"
            )
            ...
            print(
                f"  📊 升級進度: {progress} / {next_threshold if next_threshold else '計算中'} XP"
            )
        ...
        if kkcoin_gain <= 0:
            ...
            try:
                ...
                if result:
                    ...
                    print(
                        f"  📌 流入後金庫: {after_reserve} KK幣 (增加 {after_reserve - before_reserve})"
                    )
                else:
                    print("  ❌ 金庫流入失敗：add_to_central_reserve 返回 False")
        ...
        print(
            f"  ✓ 更新後資料: Lv.{updated_level}, XP: {updated_xp}, 金幣: {updated_kkcoin}"
        )
...
直接輸出情境描述，不需要任何前綴。"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    GROQ_API_URL, headers=headers, json=data, timeout=8
                ) as response:
...
直接輸出故事內容。"""
        ...
        result, provider = await complete_text_with_fallback(
            messages, max_tokens=100, nvidia_timeout=8, gemini_timeout=6, groq_timeout=6
        )
...
async def process_work_action(user_id, user_obj, action):
    ...
    try:
        ...
        result_embed = discord.Embed(
            title=result_title, description=f"*{story}*", color=result_color
        )
        ...
        if can_level_up:
            ...
            if not info["days_met"] and not info["xp_met"]:
                ...
                level_up_message = (
                    f"📈 出勤天數已達標！再累積 **{missing_xp:,}** XP 即可升級"
                )
