...
class IDDiagnosisCog(commands.Cog):
            ...
            for db_user_id, db_nickname in db_users.items():
                try:
                    ...
                    missing_members.append(
                        {"user_id": db_user_id, "db_nickname": db_nickname}
                    )
            ...
            if issues:
                ...
                for i, issue in enumerate(issues[:10], 1):  # 只顯示前 10 個
                    ...
                    if issue["type"] == "昵稱不符":
                        ...
                        report += (
                            f"   Discord 名稱: `{issue.get('discord_name', 'N/A')}`\n"
                        )
    ...
    @app_commands.command(
        name="list_id_issues", description="📋 列出所有 ID 偏差的用戶"
    )
    @app_commands.checks.has_permissions(administrator=True)
