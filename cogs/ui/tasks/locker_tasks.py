...
class LockerTasks:
                    ...
                    if found_msg:
                        ...
                        try:
                            ...
                            if success:
                                backfilled_count += 1
                                print(
                                    f"✅ 建立 canonical locker message: user {user_id}"
                                )
                            else:
                                print(
                                    f"⚠️ 建立 canonical locker message 失敗: user {user_id}"
                                )
    ...
    async def _detect_field_changes(self, user_id, user_data, message):
        ...
        try:
            ...
            embed_text = str(summary_embed.description or "") + " ".join(
                [str(f.value or "") for f in summary_embed.fields]
            )
