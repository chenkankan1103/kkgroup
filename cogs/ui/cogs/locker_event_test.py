...
class LockerEventTestCog(commands.Cog):
    ...
    @app_commands.command(
        name="test_locker_equipment", description="[測試] 觸發裝備變更事件"
    )
    ...
    async def test_equipment_event(self, interaction: discord.Interaction):
        ...
        event = EquipmentChangedEvent(
            user_id=user_id, changed_fields={"equip_0", "equip_1"}
        )
        ...
        await interaction.followup.send(
            "✅ EquipmentChangedEvent 已觸發", ephemeral=True
        )

    @app_commands.command(
        name="test_locker_currency", description="[測試] 觸發 KK幣變更事件"
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def test_currency_event(self, interaction: discord.Interaction):
        ...
        user_id = interaction.user.id
        event = CurrencyChangedEvent(user_id=user_id, changed_fields={"kkcoin", "xp"})

        print(f"🧪 [TEST] 觸發 CurrencyChangedEvent: {event}")
        self.bot.dispatch("currency_changed", event)

        await interaction.followup.send(
            "✅ CurrencyChangedEvent 已觸發", ephemeral=True
        )

    @app_commands.command(
        name="test_locker_health", description="[測試] 觸發血量變更事件"
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def test_health_event(self, interaction: discord.Interaction):
        ...
        user_id = interaction.user.id
        event = HealthChangedEvent(user_id=user_id, changed_fields={"hp", "stamina"})
    ...
    @app_commands.command(
        name="test_locker_full_refresh", description="[測試] 觸發完整刷新事件"
    )
    @app_commands.checks.has_permissions(administrator=True)
    async def test_full_refresh_event(self, interaction: discord.Interaction):
        ...
        user_id = interaction.user.id
        event = FullRefreshEvent(user_id=user_id, changed_fields={"*"})
