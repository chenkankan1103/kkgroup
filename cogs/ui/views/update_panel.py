    ...
    async def update_panel(
        ...
        if current_time - last_update_time < 5:
            ...
            await interaction.response.send_message(
                f"⏰ 請等待 {remaining_time:.1f} 秒後再更新面板！", ephemeral=True
            )
            return
        ...
        try:
            ...
            try:
                ...
                async for message in interaction.channel.history(limit=100):
                    if (
                        message.embeds
                        and message.author == self.cog.bot.user
                        and message.embeds[0].title
                        and "置物櫃" in message.embeds[0].title
                        and str(interaction.user.id)
                        in str(message.embeds[0].description or "")
                    ):
                        try:
                            await message.edit(embed=embed, view=self)
                            await interaction.followup.send(
                                "✅ 面板已更新！", ephemeral=True
                            )
                            return
                        except:
                            continue
