...
class GenderSelectView(discord.ui.View):
    ...
    async def gender_select(
        ...
        await interaction.followup.send(
            f"✅ 性別已設定為{gender_text}！", ephemeral=True
        )
...
class WorkCardModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.followup.send(
                "✅ 員工證已領取！該卡片已保存到你的置物櫃。", ephemeral=True
            )
            ...
            traceback.print_exc()
            await interaction.followup.send(
                f"❌ 提交失敗：{str(e)[:100]}", ephemeral=True
            )
    ...
    async def create_work_card_embed(self, user_data, user_obj):
        ...
        # 計算下一級 XP
        next_level_xp = (
            LEVELS.get(level + 1, {}).get("xp_required", xp) if level < 6 else xp
        )
        ...
        embed.add_field(
            name="📊 入職統計", value=f"連勤天數：{streak} 天 🔥", inline=False
        )
...
class WorkCardActionView(PersistentViewBase):
    ...
    @discord.ui.button(
        ...
        emoji="🎫",
        custom_id="view_my_card",
    )
    async def view_card_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        """查看員工證"""
        if interaction.user.id != self.user_id:
        ...
        try:
            user_obj = await self.cog.bot.fetch_user(self.user_id)
            embed = await WorkCardModal(self.cog, self.user_id).create_work_card_embed(
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.primary,
        emoji="✏️",
        custom_id="modify_card_settings",
    )
