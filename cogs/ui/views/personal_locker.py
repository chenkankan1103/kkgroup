...
class PersonalLockerView(discord.ui.View):
    ...
    async def crop_planting_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            try:
                ...
                if not inventory:
                    print(
                        f"⚠️  [Crop Planting] Failed to get inventory for user {self.user_id}"
                    )
                    await interaction.followup.send(
                        "❌ 無法獲取庫存資料！請稍後再試。", ephemeral=True
                    )
                    return
            except Exception as inv_error:
                print(
                    f"❌ [Crop Planting] Inventory error for user {self.user_id}: {inv_error}"
                )
                traceback.print_exc()
                await interaction.followup.send(
                    "❌ 獲取庫存時發生錯誤！請聯繫管理員。", ephemeral=True
                )
                return
            ...
            if not seeds or not any(qty > 0 for qty in seeds.values()):
                await interaction.followup.send(
                    "❌ 你沒有種子！請先到商店購買種子。", ephemeral=True
                )
                return
            ...
            for seed_name, qty in seeds.items():
                if qty > 0:
                    try:
                        ...
                        print(
                            f"⚠️  [Crop Planting] Seed type '{seed_name}' not found in CANNABIS_SHOP"
                        )
                        continue
            ...
            view = SelectSeedView(
                self.bot, self.cog, self.user_id, self.guild_id, self.channel_id, seeds
            )
            # 在 defer 後使用 followup.send() 而不是 edit_message()
            await interaction.followup.send(embed=embed, view=view, ephemeral=True)
            add_log(
                "ui",
                f"[Crop Planting] Seed selection view updated for user {self.user_id}",
            )

        except Exception as e:
            add_log(
                "ui", f"[Crop Planting] Unexpected error for user {self.user_id}: {e}"
            )
            traceback.print_exc()
            try:
                await interaction.followup.send(
                    f"❌ 發生錯誤：{str(e)[:100]}", ephemeral=True
                )
            except:
                pass
                ...
                for idx, plant in enumerate(growing, 1):
                    ...
                    if plant["status"] == "harvested":
                        ...
                        if isinstance(planted_time, str):
                            planted_time = datetime.fromisoformat(
                                planted_time
                            ).timestamp()
                        if isinstance(matured_time, str):
                            matured_time = datetime.fromisoformat(
                                matured_time
                            ).timestamp()
    ...
    async def personal_items_callback_impl(self, interaction: discord.Interaction):
        ...
        try:
            ...
            # 顯示數位美金（在最上面）
            embed.add_field(
                name="💵 數位美金", value=f"**${digital_usd:,.2f} USD**", inline=False
            )
            ...
            if inventory.get("種子"):
                ...
                if seed_info:
                    embed.add_field(
                        name="🌱 種子", value=seed_info.strip(), inline=True
                    )
            ...
            if inventory.get("大麻"):
                ...
                if cannabis_info:
                    embed.add_field(
                        name="📦 大麻", value=cannabis_info.strip(), inline=True
                    )
        ...
        try:
            ...
            try:
                ...
                if (
                    interaction.message
                    and locker_msg_id
                    and interaction.message.id == locker_msg_id
                ):
...
class WeeklySummaryCannabisPanelView(discord.ui.View):
    ...
    async def harvest_button(
        ...
        try:
            ...
            # 顯示可收割的植物
            embed = discord.Embed(
                title="🔪 選擇要收割的植物", color=discord.Color.gold()
            )
    ...
    @discord.ui.button(
        ...
        style=discord.ButtonStyle.secondary,
        emoji="📊",
        custom_id="weekly_view_plants",
    )
    async def view_plants_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        """查看植物狀態"""
        try:
            ...
            if not plants:
                await interaction.followup.send(
                    "❌ 你還沒有種植任何植物！", ephemeral=True
                )
                return

            embed = discord.Embed(title="🌱 我的植物狀態", color=discord.Color.green())
            ...
            for idx, plant in enumerate(plants, 1):
                ...
                if plant["status"] == "harvested":
                    ...
                    planted_time = (
                        plant["planted_at"]
                        if isinstance(plant["planted_at"], float)
                        else plant["planted_at"]
                    )
                    matured_time = (
                        plant["matured_at"]
                        if isinstance(plant["matured_at"], float)
                        else plant["matured_at"]
                    )
                ...
                embed.add_field(
                    name=f"#{idx} {seed_config['emoji']}", value=value, inline=False
                )
