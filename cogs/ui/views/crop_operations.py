...
class CropOperationView(discord.ui.View):
    ...
    def __init__(
        self, bot, cog, user_id, guild_id, channel_id, seeds, plants, growing, harvested
    ):
    ...
    async def crop_harvest_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            view = SelectPlantForHarvestView(
                self.bot,
                self.cog,
                self.user_id,
                self.guild_id,
    ...
    async def crop_harvest_all_callback(self, interaction: discord.Interaction):
        ...
        try:
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        harvest_results.append(
                            {"plant": plant_name, "yield": yield_amount}
                        )
                        ...
                        if self.cog:
                            ...
                            await self.cog.record_event(
                                "harvest", user, f"收割{plant_name}獲得{yield_amount}個"
                            )
            ...
            # 🚫 移除自動計價邏輯，避免誤導
            embed.add_field(
                name="💰 出售方式", value="請到商人處出售大麻以獲得 KK幣", inline=False
            )
            ...
            await interaction.followup.send(
                f"❌ 一鍵收割時發生錯誤：{str(e)[:100]}", ephemeral=True
            )
    ...
    async def back_to_locker_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if fertilizers:
                fert_info = ", ".join(
                    [f"{k}: {v}" for k, v in fertilizers.items() if v > 0]
                )
            ...
            traceback.print_exc()
            await interaction.followup.send(
                f"❌ 返回時發生錯誤：{str(e)[:100]}", ephemeral=True
            )

    @classmethod
    async def create_crop_info_embed_and_view(
        cls, bot, cog, user_id, guild_id, channel_id
    ):
        ...
        # 創建CropOperationView
        view = cls(
            bot, cog, user_id, guild_id, channel_id, seeds, plants, growing, harvested
        )
...
class CropPlantingView(discord.ui.View):
        ...
        try:
            ...
            async def plant_single_callback(btn_interaction: discord.Interaction):
                ...
                try:
                    ...
                    has_seed = await remove_inventory(
                        self.user_id, "種子", selected_seed, 1
                    )
                    if not has_seed:
                    ...
                    result = await plant_cannabis(
                        self.user_id, self.guild_id, self.channel_id, selected_seed
                    )
                    ...
                    if result and not result.get("success") == False:
                        ...
                        embed.add_field(
                            name="成長時間",
                            value=f"{config['growth_time']//3600} 小時",
                            inline=False,
                        )
                        embed.add_field(
                            name="最大產量",
                            value=f"{config['max_yield']} 個",
                            inline=False,
                        )

                        if self.cog:
                            await self.cog.record_event(
                                "plant", btn_interaction.user, f"種植{selected_seed}"
                            )
                        ...
                        reason = (
                            result.get("reason", "未知原因") if result else "未知原因"
                        )
                        await btn_interaction.followup.send(
                            f"❌ 種植失敗：{reason}", ephemeral=True
                        )
            ...
            plant_all_btn = discord.ui.Button(
                label="配置一鍵種植", style=discord.ButtonStyle.primary, emoji="⚙️"
            )

            async def plant_all_from_select_callback(
    ...
    async def config_plant_all_callback(self, interaction: discord.Interaction):
        ...
        try:
            ...
            confirm_btn = discord.ui.Button(
                label="確認種植", style=discord.ButtonStyle.success, emoji="✅"
            )
    ...
    async def update_config_embed(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.add_field(
                name="📊 合計", value=f"將種植 {total_selected} 棵植物", inline=False
            )
            embed.add_field(
                name="💡 說明",
                value="使用 +/- 按鈕調整數量，完成後點「確認種植」",
    ...
    async def execute_configured_planting(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if remaining_slots <= 0:
                await interaction.followup.send(
                    "❌ 植物格位已滿（5/5），無法再種植！", ephemeral=True
                )
                return
            ...
            for seed_name, qty_to_plant in self.plant_all_config.items():
                for _ in range(qty_to_plant):
                    ...
                    result = await plant_cannabis(
                        self.user_id, self.guild_id, self.channel_id, seed_name
                    )
                    if result and not result.get("success") == False:
                        if self.cog:
                            user = await self.bot.fetch_user(self.user_id)
                            await self.cog.record_event(
                                "plant", user, f"種植{seed_name}"
                            )
            ...
            # 建立結果 embed
            embed = discord.Embed(
                title="🌱 配置一鍵種植結果", color=discord.Color.green()
            )
            embed.add_field(
                name="📊 種植進度",
                value=f"{plants_count}/{remaining_slots} 個植物已種植",
                inline=False,
            )
            ...
            for seed_name, success, reason in results:
                ...
                if success:
                    embed.add_field(
                        name=f"{emoji} {seed_name}", value="✅ 種植成功", inline=True
                    )
                else:
                    embed.add_field(
                        name=f"{emoji} {seed_name}", value=f"❌ {reason}", inline=True
                    )
            ...
            traceback.print_exc()
            await interaction.followup.send(
                f"❌ 種植時發生錯誤：{str(e)[:100]}", ephemeral=True
            )
...
class SelectSeedView(discord.ui.View):
    ...
    def __init__(
        self, bot, cog, user_id, guild_id, channel_id, seeds, crop_operation_view=None
    ):
        super().__init__(timeout=None)  # 永久視圖
        self.bot = bot
        self.cog = cog
        ...
        async def callback(interaction: discord.Interaction):
            try:
                ...
                if result and not result.get("success") == False:
                    ...
                    embed.add_field(
                        name="最大產量", value=f"{config['max_yield']} 個", inline=False
                    )
