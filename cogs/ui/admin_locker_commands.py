...
class AdminLockerCommands(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        # 檢查是否為 cron 腳本發送的觸發訊息
        if message.content.strip() == "定時任務觸發：批量刷新置物櫃":
            logger.info("📡 收到置物櫃自動刷新觸發訊息，開始執行...")
            await self.refresh_all_lockers()
            # 可選：在特定頻道發送執行結果
            # await self.send_log_to_channel(message.channel, success_count, fail_count, total)
