...
class AdminLockerCommands(commands.Cog):
    ...
    async def refresh_all_lockers(self):
        ...
        try:
            ...
            if not user_panel_cog:
                logger.warning(
                    "⚠️ 找不到 UserPanel cog，將跳過「新增置物櫃」邏輯，僅更新已有置物櫃"
                )

            # FORUM_CHANNEL_ID：優先從 UserPanel 取，否則從環境變數取
            import os

            forum_channel_id = getattr(user_panel_cog, "FORUM_CHANNEL_ID", None) or int(
                os.getenv("FORUM_CHANNEL_ID", "0")
            )
                ...
                try:
                    ...
                    if thread_id:
                        ...
                        try:
                            ...
                            if thread:
                                ...
                                if locker_message_id:
                                    try:
                                        message_obj = await thread.fetch_message(
                                            locker_message_id
                                        )
                                    except discord.NotFound:
                                        message_obj = None
                                ...
                                fail_count += 1
                                logger.warning(
                                    f"⚠️ 用戶 {user_id} 的 thread 無效，已重設"
                                )
                        ...
                        if not user_panel_cog:
                            ...
                            fail_count += 1
                            logger.debug(
                                f"⚠️ 用戶 {user_id} 無置物櫃且 UserPanel 不可用，跳過"
                            )
                        else:
                            try:
                                ...
                                if user:
                                    ...
                                    thread = (
                                        await user_panel_cog.get_or_create_user_thread(
                                            user
                                        )
                                    )
                                    if thread:
                                        success_count += 1
                                        logger.debug(
                                            f"✅ 已為用戶 {user_id} 創建新置物櫃"
                                        )
                                    else:
                                        fail_count += 1
                                        logger.warning(
                                            f"⚠️ 用戶 {user_id} 的置物櫃建立失敗（無法建立）"
                                        )
                                else:
                                    fail_count += 1
                            except discord.NotFound:
                                ...
                                fail_count += 1
                                logger.warning(
                                    f"⚠️ 建立用戶 {user_id} 置物櫃失敗: {user_err}"
                                )
            ...
            logger.info(
                f"✅ 【批量刷新】完成：{success_count}/{total} 成功 (成功率 {success_rate:.1f}%)"
            )
    ...
    async def admin_refresh_all_lockers(self, interaction: discord.Interaction):
        ...
        try:
            ...
            embed.set_footer(
                text=f"執行者：{interaction.user.name} | 時間：{discord.utils.utcnow().strftime('%Y-%m-%d %H:%M:%S UTC')}"
            )
    ...
    async def on_message(self, message):
        ...
        if message.content.strip() == "定時任務觸發：批量刷新置物櫃":
            ...
            if allowed_ids and message.channel.id not in allowed_ids:
                return
            logger.info(
                f"📡 收到置物櫃自動刷新觸發訊息（頻道 {message.channel.id}），開始執行..."
            )
            try:
