...
class ScamParkEvents(commands.Cog):
    def __init__(self, bot):
        ...
        if self.AI_API_KEY and "generativelanguage.googleapis.com" in (
            self.AI_API_URL or ""
        ):
    ...
    def load_last_event_times(self):
        ...
        try:
            ...
            print(
                f"✅ 初始化 {len(self.event_cooldown)} 個用戶的事件冷卻時間（允許立即觸發）"
            )
        except Exception as e:
            print(f"❌ 載入事件歷史錯誤: {e}")

    def save_event_time(
        self, user_id: int, message_id: int = None, event_type: str = None
    ):
        """儲存事件觸發時間到記憶體"""
        try:
            current_time = datetime.datetime.now().timestamp()
            ...
            if user_id in self.event_history:
                ...
                self.event_history[user_id]["event_count"] = (
                    self.event_history[user_id].get("event_count", 0) + 1
                )
        ...
        try:
            ...
            for user_data in all_users:
                ...
                if random.random() < 0.05:  # 只記錄5%的檢查
                    print(
                        f"🔍 檢查用戶 {user_id}: 距上次 {time_since_last/3600:.1f}h, "
                ...
                if random.random() < trigger_chance:
                    ...
                    if thread and member:
                        print(
                            f"✅ 觸發事件: 用戶 {member.name}, 距上次 {time_since_last/3600:.1f}小時"
                        )
                        await self.trigger_random_event(
                            member, thread, kkcoin, level, hp, stamina
                        )
    ...
    async def generate_pollinations_image(
        self, prompt: str, is_negative_event: bool = True, scene_variety: int = None
    ) -> str:
        ...
        try:
            ...
            if is_negative_event:
                ...
                color_tone = random.choice(
                    ["warm subtle lighting", "soft natural light", "golden hour glow"]
                )
        ...
        try:
            ...
            data = {
                ...
                "messages": [
                    ...
                    {
                        "role": "user",
                        "content": f"Translate this to English for image generation: {chinese_text}",
                    },
                ],
                "max_tokens": 100,
                "temperature": 0.3,
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.GROQ_API_URL, headers=headers, json=data, timeout=10
                ) as response:
                    if response.status == 200:
                        result = await response.json()
    ...
    async def generate_ai_event_description(
...
要讓人感受到詐騙園區的壓迫感和絕望氛圍。"""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.post(
                    self.AI_API_URL, headers=headers, json=data, timeout=10
                ) as response:
                    if response.status == 200:
                        result = await response.json()
                        return result["choices"][0]["message"]["content"].strip()
    ...
    async def send_or_edit_event_message(
        self,
        ...
        try:
            ...
            try:
                event_history = (
                    json.loads(event_history_str)
                    if isinstance(event_history_str, str)
                    else []
                )
            ...
            new_event = {"type": event_type, "time": current_time}
            event_history.insert(0, new_event)  # 新事件放在最前
            event_history = event_history[:5]  # 只保留前 5 條

            # 保存事件歷史到數據庫
            set_user_field(
                user_id, "event_history", json.dumps(event_history, ensure_ascii=False)
            )
            ...
            # 添加歷史欄位到 embed
            embed.add_field(name="📊 你的事件歷史", value=history_text, inline=False)

            # 檢查是否有之前的事件消息
            last_message_id = get_user_field(
                user_id, "last_event_message_id", default=None
            )
            message = None
            ...
            if last_message_id:
                try:
                    ...
                    print(
                        f"⚠️ 舊事件消息已被刪除 (id={last_message_id})，清除並發送新訊息"
                    )
                    set_user_field(user_id, "last_event_message_id", None)
                except discord.Forbidden:
                    print("⚠️ 無法編輯事件消息 (Forbidden)，發送新訊息")
                    set_user_field(user_id, "last_event_message_id", None)
                except discord.HTTPException as e:
                    print(
                        f"⚠️ 編輯消息 HTTP 錯誤 {e.status}/{e.code}: {e.text}，嘗試發送新訊息"
                    )
                except Exception as e:
        ...
        try:
            ...
            if member.id in self.event_messages:
                ...
                try:
                    ...
                    if old_message.created_at.timestamp() > (
                        datetime.datetime.now().timestamp() - 14400
                    ):
    ...
    def get_possible_events(
        self, kkcoin: int, level: int, hp: int, stamina: int
    ) -> list:
        """根據使用者狀態獲取可能的事件"""
        events = []
        ...
        events.extend(
            [
                ...
                {
                    "handler": self.event_cannabis_confiscation,
                    "weight": 3,
                },  # 大麻檢查事件
    ...
    async def event_major_confiscation(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """大額搜查行動"""
        ...
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "重大搜查"
        )
    ...
    async def event_protection_fee(self, member, thread, kkcoin, level, hp, stamina):
        ...
        image_url = await self.generate_pollinations_image(
            image_prompt
            or "scam park boss collecting protection money, threatening atmosphere",
            is_negative_event=True,
        )
        ...
        embed = discord.Embed(
            title="💰 保護費收取",
            description=ai_desc
            or "「想要好好工作？那就得付點保護費...」園區老大笑著說。",
            color=0xFF6600,
            timestamp=discord.utils.utcnow(),
        )
        embed.add_field(name="💸 本月保護費", value=f"-{fee} KKCoin", inline=False)
        embed.add_field(
            name="� 保護範圍", value="工作安全、人身安全、不會被欺負", inline=False
        )
    ...
    async def event_beating(self, member, thread, kkcoin, level, hp, stamina):
        ...
        image_url = await self.generate_pollinations_image(
            image_prompt
            or "scam park supervisor violently punishing worker, dark atmosphere",
            is_negative_event=True,
        )
    ...
    async def event_bribe_demand(self, member, thread, kkcoin, level, hp, stamina):
        ...
        embed = discord.Embed(
            title="🤝 主管「建議」",
            description="主管拍了拍你的肩膀：「最近查得很嚴啊，要不要意思意思？」",
            color=0xAA6600,
            timestamp=discord.utils.utcnow(),
        )
        embed.add_field(name="💵 「建議金額」", value=f"-{bribe} KKCoin", inline=False)
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "主管暗示"
        )
    ...
    async def event_gambling_trap(self, member, thread, kkcoin, level, hp, stamina):
        ...
        embed.add_field(
            name="✅ 聰明選擇", value="虧得你沒參加，聽說裡面全是老千。", inline=False
        )
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "賭局拒絕"
        )
    ...
    async def event_equipment_fee(self, member, thread, kkcoin, level, hp, stamina):
        ...
        embed.add_field(
            name="📢 提醒", value="維護期間可能會有短暫不便，敬請見諒", inline=False
        )
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "設備維護"
        )

    async def event_kkcoin_confiscation(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """房間檢查"""
        embed = discord.Embed(
    ...
    async def event_forced_overtime(self, member, thread, kkcoin, level, hp, stamina):
        ...
        ai_desc = await self.generate_ai_event_description(
            "加班", {"損失體力": stamina_loss, "賺得": kkcoin_gain}
        )

        image_prompt = await self.translate_to_english(
            "員工被迫在辦公室一直工作到天亮，疲勞不堪"
        )
        image_url = await self.generate_pollinations_image(
            image_prompt
            or "employee forced overtime working exhausted at desk until dawn",
            is_negative_event=True,
        )
        ...
        embed.add_field(
            name="😫 結果", value="整個人都掏空了，明天可能會很難受...", inline=False
        )
    ...
    async def event_blackmail(self, member, thread, kkcoin, level, hp, stamina):
        ...
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "謠言"
        )

    async def event_supervisor_inspection(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """主管巡視"""
        stamina_loss = random.randint(10, 20)
    ...
    async def event_training_hell(self, member, thread, kkcoin, level, hp, stamina):
        ...
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "地獄訓練"
        )
        self.update_user_stats(member.id, hp=-hp_loss, stamina=-stamina_loss)

    async def event_isolation_punishment(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """關禁閉"""
    ...
    async def event_car_accident(self, member, thread, kkcoin, level, hp, stamina):
        ...
        ai_desc = await self.generate_ai_event_description(
            "車禍", {"傷害": hp_loss, "體力消耗": stamina_loss}
        )
        ...
        embed.add_field(name="⚡ 體力消耗", value=f"-{stamina_loss} 體力", inline=True)
        embed.add_field(
            name="🏥 狀態", value="痛到不敢走路，得休息好幾天...", inline=False
        )
    ...
    async def event_food_poisoning(self, member, thread, kkcoin, level, hp, stamina):
        ...
        embed.add_field(name="⚡ 精力耗盡", value=f"-{stamina_loss} 體力", inline=True)
        embed.add_field(
            name="💊 治療", value="得吃止瀉藥和電解質飲料才能恢復...", inline=False
        )
    ...
    async def event_workplace_illness(self, member, thread, kkcoin, level, hp, stamina):
        ...
        ai_desc = await self.generate_ai_event_description(
            "工作病", {"病症": illness_type, "體力下降": stamina_loss}
        )
        ...
        embed.add_field(name="⚡ 精神崩潰", value=f"-{stamina_loss} 體力", inline=True)
        embed.add_field(
            name="🏥 醫囑", value="醫生說得好好休息，不然會更嚴重...", inline=False
        )
        ...
        message = await self.send_or_edit_event_message(
            thread, embed, member.id, "過勞成疾"
        )
        self.update_user_stats(member.id, hp=-hp_loss, stamina=-stamina_loss)

    async def event_workplace_accident(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """工作場所傷害"""
        ...
        image_url = await self.generate_pollinations_image(
            image_prompt or "workplace accident, injured worker", is_negative_event=True
        )
        ...
        embed.add_field(
            name="📋 工傷", value="得去醫院掛號，可能會請病假...", inline=False
        )
    ...
    def update_user_stats(
        self, user_id: int, kkcoin: int = 0, hp: int = 0, stamina: int = 0
    ):
    ...
    async def event_cannabis_confiscation(
        self, member, thread, kkcoin, level, hp, stamina
    ):
        """大麻沒收事件 - 檢查正在種植的植物"""
        try:
            ...
            active_plants = [
                plant for plant in plants if plant.get("status") != "harvested"
            ]

            if not active_plants:
                # 如果沒有正在種植的植物，改為一般檢查通知
                ai_desc = await self.generate_ai_event_description(
                    "置物櫃檢查", {"發現": "無違禁品", "原因": "隨機檢查"}
                )
                ...
                embed.add_field(
                    name="✅ 檢查結果", value="無違禁品，檢查通過", inline=False
                )

                message = await self.send_or_edit_event_message(
                    thread, embed, member.id, "置物櫃檢查"
                )
                # 記錄執行時間
            ...
            for plant in active_plants:
                try:
                    ...
                    if plant_id:
                        ...
                        destroyed_plants.append(
                            f"{seed_type} (成長度: {progress:.1f}%)"
                        )
                        total_destroyed += 1
                except Exception as e:
                    print(f"❌ 摧毀植物失敗: {e}")
            ...
            ai_desc = await self.generate_ai_event_description(
                "大麻種植摧毀", {"摧毀植物": destroyed_plants, "總數": total_destroyed}
            )
            ...
            embed.add_field(
                name="⚠️ 嚴重警告", value="下次再犯將會被關禁閉！", inline=False
            )
