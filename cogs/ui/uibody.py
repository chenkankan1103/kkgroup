...
class UserPanel(commands.Cog):
    ...
    async def restore_image_cache_from_storage(self):
        """啟動時恢復圖片快取"""
        await restore_image_cache_from_storage(
            self.bot, self.image_cache, self.image_storage_channel_id
        )
    ...
    async def create_threads_for_existing_members(self):
        ...
        try:
            ...
            if threads_to_create:
                print(
                    f"🔧 發現需要創建的線程: {len(threads_to_create)} 個 (已有線程: {existing_threads})"
                )

                for member in threads_to_create:
                    try:
                        thread = await self.get_or_create_user_thread(
                            member, skip_image_on_startup=True
                        )
                        if thread:
                            await asyncio.sleep(1)
                    except discord.HTTPException as e:
                        if e.status == 429:
                            print("⚠️ 遇到速率限制，稍候 30 秒...")
                            await asyncio.sleep(30)
                            try:
                                await self.get_or_create_user_thread(
                                    member, skip_image_on_startup=True
                                )
    ...
    async def get_or_create_user_thread(
        self, user: discord.User, skip_image_on_startup: bool = False
    ...
    async def generate_ai_comment(
        ...
        """生成 AI 評論（只使用 Groq API）"""
        try:
            if self.GROQ_API_KEY and self.GROQ_API_URL:
                comment = await self._generate_groq_comment(
                    member, kkcoin_change, xp_change, level_change
                )
                if comment:
                    return comment
        except Exception:
        ...
        return "本週表現不錯！繼續保持這個節奏 💪"

    async def _generate_groq_comment(
        self,
        member: discord.Member,
        kkcoin_change: int,
        xp_change: int,
            ...
            for user in all_users:
                ...
                if user.get("work_card_enabled"):
                    try:
                        ...
                        logger.warning(
                            f"⚠️ 註冊用戶 {user.get('user_id')} 的員工證 View 失敗: {e}"
                        )
                        continue
        ...
        try:
            ...
            for user_data in all_users:
                ...
                try:
                    ...
                    ai_comment = await self.generate_ai_comment(
                        member, kkcoin_change, xp_change, level_change
                    )
                    ...
                    if kkcoin_change != 0:
                        embed.add_field(
                            name="💰 KKCoin 變動",
                            value=f"{kkcoin_change:+}",
                            inline=True,
                        )
                    if xp_change != 0:
                        embed.add_field(
                            name="✨ 經驗值 變動", value=f"{xp_change:+}", inline=True
                        )
                    if level_change != 0:
                        embed.add_field(
                            name="⭐ 等級 變化", value=f"{level_change:+}", inline=True
                        )

                    if ai_comment:
                        embed.add_field(
                            name="🤖 AI 評論", value=ai_comment, inline=False
                        )
                    ...
                    try:
                        ...
                        if e.code == 50083:  # Thread is archived
                            print(
                                f"[WEEKLY SUMMARY] Thread {thread.id} is archived, skipping"
                            )
                            continue
                        raise
                    await asyncio.sleep(1)
        ...
        try:
            ...
            if member_role_id not in before_ids and member_role_id in after_ids:
                ...
                if self.ensure_user_exists(after.id):
                    ...
                    await self.get_or_create_user_thread(
                        after, skip_image_on_startup=True
                    )
        except Exception:
            pass
