...
async def create_user_embed(cog, user_data: dict, user: discord.User) -> discord.Embed:
    ...
    try:
        ...
        if api_url:
            ...
            print(
                f"[embed_utils] ⚠️ paperdoll_manager 返回 None (用戶: {user_data.get('user_id')})"
            )
            print(
                f"[embed_utils]    用戶數據: {user_data.get('face', 'N/A')}, {user_data.get('hair', 'N/A')}"
            )
    except Exception as e:
        import traceback

        print(
            f"[embed_utils] ❌ 添加紙娃娃圖片失敗 (用戶: {user_data.get('user_id')}): {e}"
        )
    ...
    embed.add_field(
        name="💰 金錢", value=f"{user_data['kkcoin'] or 0} KKCoin", inline=True
    )
    embed.add_field(
        name="💵 數位美金",
        value=f"${float(user_data.get('digital_usd') or 0):,.2f}",
        inline=True,
    )
    ...
    try:
        ...
        for seed_name, qty in (cannabis_inv.get("種子") or {}).items():
            if qty and qty > 0:
                emoji = (
                    CANNABIS_SHOP.get("種子", {}).get(seed_name, {}).get("emoji", "🌱")
                )
        ...
        for fert_name, qty in (cannabis_inv.get("肥料") or {}).items():
            if qty and qty > 0:
                emoji = (
                    CANNABIS_SHOP.get("肥料", {}).get(fert_name, {}).get("emoji", "🧂")
                )
