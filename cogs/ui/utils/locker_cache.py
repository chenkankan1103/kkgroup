...
class LockerCache:
    ...
    async def get_paperdoll_image(
        self, user_data: dict, force_refresh: bool = False
    ) -> Optional[str]:
        """
        獲取紙娃娃圖片 URL
        ...
        if not force_refresh and current_hash in self.paperdoll_cache:
            ...
            if now - timestamp < self.cache_ttl:
                self.hit_count += 1
                print(
                    f"✅ [LockerCache] 命中: hash={current_hash[:8]}... (age: {now - timestamp:.0f}s)"
                )
                return cached_url
            else:
                # 快取過期
    ...
    def clear_all(self) -> None:
        ...
        print(
            f"🗑️ [LockerCache] 清除所有快取 (命中: {self.hit_count}, 未命中: {self.miss_count})"
        )
