...
async def generate_canonical_locker_embed(
    ...
    try:
        if hasattr(cog, "create_user_embed"):
            print("[Locker]   → 使用 cog.create_user_embed()")
            embed = await cog.create_user_embed(user_data, user_obj)
        else:
            # 嘗試從 utils 導入
            print("[Locker]   → 使用 embed_utils.create_user_embed()")
            from .embed_utils import create_user_embed as util_create

            embed = await util_create(
                cog, user_data, user_obj or discord.Object(id=user_id)
            )
        print("[Locker]   ✅ Step 1 成功：已生成主 Embed")
        ...
        if not img_url:
            ...
            try:
                ...
                if api_url:
                    ...
                    print("[Locker]   → 呼叫 embed.set_image()")
                    embed.set_image(url=api_url)
                    print("[Locker]   ✅ 成功調用 set_image()")
                else:
                    print("[Locker]   ❌ build_api_url() 返回 None")
    ...
    if not embed:
        ...
        try:
            user_name = (
                (await cog.bot.fetch_user(user_id)).name
                if hasattr(cog, "bot")
                else f"用戶{user_id}"
            )
        except Exception:
        ...
        print("[Locker] 🆘 Step 3：主 Embed 生成失敗，建立 Fallback Embed")
        embed = discord.Embed(
        ...
        try:
            ...
            if api_url:
                ...
                embed.set_image(url=api_url)
                print("[Locker]   ✅ Step 3 成功添加紙娃娃圖片")
                embed.set_footer(text="💫 由 MapleStory.io API 提供角色外觀")
            else:
                print("[Locker]   ❌ Step 3 build_api_url() 返回 None")
        except Exception as img_err:
    ...
    if include_cannabis_info and plants:
        ...
        for plant in plants:
            ...
            embed.add_field(
                name=f"植物 #{plant.get('id')}", value=field_value, inline=False
            )
    ...
    if include_cannabis_info and inventory:
        ...
        if inventory.get("種子"):
            seeds_info = "".join(
                f"  🌱 {k} x{v}\n"
                for k, v in (inventory.get("種子") or {}).items()
                if v
            )
            if seeds_info:
                embed.add_field(
                    name="🌾 種子庫存", value=seeds_info.strip(), inline=True
                )

        if inventory.get("肥料"):
            fert_info = "".join(
                f"  💧 {k} x{v}\n"
                for k, v in (inventory.get("肥料") or {}).items()
                if v
            )
            if fert_info:
                embed.add_field(
                    name="💧 肥料庫存", value=fert_info.strip(), inline=True
                )

        if inventory.get("大麻"):
            cannabis_info = "".join(
                f"  💰 {k} x{v} ({CANNABIS_HARVEST_PRICES.get(k,0)}/個)\n"
                for k, v in (inventory.get("大麻") or {}).items()
                if v
            )
            if cannabis_info:
                embed.add_field(
                    name="📦 大麻庫存", value=cannabis_info.strip(), inline=False
                )
        ...
        for row in message.components or []:
            for child in getattr(row, "children", []):
                cid = getattr(child, "custom_id", None) or (
                    child.get("custom_id") if isinstance(child, dict) else None
                )
                if cid == "locker_crop_info":
                    return True
    ...
    try:
        ...
        if getattr(thread, "archived", False):
            try:
                ...
                print(
                    f"❌ [Update Locker Message] 無法解除封存 thread {thread.id}: {unarchive_err}"
                )
                return False
                ...
                async for msg in thread.history(limit=30, oldest_first=True):
                    if msg.author.id == bot.user.id and msg.embeds:
                        ...
                        try:
                            ...
                            pass
                        print(
                            f"🔍 [Update Locker Message] 在 thread 中找到已有 embed 訊息 {msg.id}，將直接編輯"
                        )
                        break
        ...
        if message_obj:
            try:
                ...
                print("⚠️ [Update Locker Message] 訊息已被刪除，將發送新訊息")
                message_obj = None
            except Exception as e:
        ...
        if restore_archived:
            try:
                ...
                print(
                    f"⚠️ [Update Locker Message] 恢復封存失敗 thread {thread.id}: {archive_err}"
                )
