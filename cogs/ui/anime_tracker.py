...
from .push_core import (ANIME_CHANNEL_ID, API_ENDPOINT, API_HEADERS,
                        API_TIMEOUT, TW_TZ, AnimePushCore, get_week_start_date)
...
class AnimeTracker(commands.Cog):
    ...
    async def set_dependencies(self, db_path: str):
        ...
        # 初始化排程器
        print(
            f"[AnimeTracker.set_dependencies] _scheduler_started={self._scheduler_started}",
            flush=True,
        )
        if not self._scheduler_started:
            print(
                "[AnimeTracker.set_dependencies] 呼叫 _init_scheduler()...", flush=True
            )
        ...
        # 檢查並初始化週表（如果為空）
        print(
            "[AnimeTracker.set_dependencies] 呼叫 _init_weekly_schedule_if_empty()...",
            flush=True,
        )
    ...
    async def cog_load(self):
        ...
        self.logger.info(
            "🚀 [AnimeTracker.cog_load] AnimeTracker Cog 載入完成（等待 set_dependencies 初始化依賴）"
        )
    ...
    async def _refresh_weekly_schedule_task(self):
        ...
        try:
            self.logger.info(
                "🔄 [AnimeTracker._refresh_weekly_schedule_task] 開始週表資料刷新"
            )
            result = await self.schedule_tracker.refresh_weekly_schedule()
            if result.get("success"):
                self.logger.info(
                    f"✅ [AnimeTracker._refresh_weekly_schedule_task] 週表資料刷新成功"
                )
                # 重新排程推送任務
                await self._reschedule_push_jobs()
            else:
                self.logger.error(
                    f"❌ [AnimeTracker._refresh_weekly_schedule_task] 週表資料刷新失敗: {result.get('error')}"
                )
        except Exception as e:
            self.logger.error(
                f"❌ [AnimeTracker._refresh_weekly_schedule_task] 週表資料刷新異常: {e}",
                exc_info=True,
            )
    ...
    async def _sync_episode_stats_task(self):
        ...
        try:
            self.logger.info(
                "🔄 [AnimeTracker._sync_episode_stats_task] 開始同步動畫統計"
            )
            await self.ranking_stats.sync_episode_stats()
            await self.ranking_stats.update_weekly_stats()
            self.logger.info(
                "✅ [AnimeTracker._sync_episode_stats_task] 動畫統計同步完成"
            )
        except Exception as e:
            self.logger.error(
                f"❌ [AnimeTracker._sync_episode_stats_task] 動畫統計同步失敗: {e}",
                exc_info=True,
            )
    ...
    async def _init_weekly_schedule_if_empty(self):
        ...
        try:
            ...
            if not today_schedule:
                ...
                if result.get("success"):
                    self.logger.info(
                        "✅ [_init_weekly_schedule_if_empty] 週表初始化完成"
                    )
                else:
                    self.logger.error(
                        f"❌ [_init_weekly_schedule_if_empty] 週表初始化失敗: {result.get('error')}"
                    )
            else:
                self.logger.info(
                    f"📅 [_init_weekly_schedule_if_empty] 週表已有資料 ({len(today_schedule)} 筆)，跳過 API 拉取"
                )
            ...
            if "no such table: anime_weekly_schedule" in str(e):
                self.logger.warning(
                    "⚠️ [_init_weekly_schedule_if_empty] 週表尚未建立，跳過初始化"
                )
            else:
                self.logger.error(
                    f"❌ [_init_weekly_schedule_if_empty] 初始化週表時發生資料庫錯誤: {e}",
                    exc_info=True,
                )
        except Exception as e:
            self.logger.error(
                f"❌ [_init_weekly_schedule_if_empty] 初始化週表時發生錯誤: {e}",
                exc_info=True,
            )
    ...
    async def _reschedule_push_jobs(self):
        ...
        try:
            ...
            if push_job_ids:
                self.logger.info(
                    f"🧹 [_reschedule_push_jobs] 已移除 {len(push_job_ids)} 個舊推送任務"
                )
            ...
            for schedule_item in today_schedule:
                ...
                anime_sn = (
                    schedule_item.get("anime_sn") or 0
                )  # anime_sn 可能為 None，預設為 0
                ...
                try:
                    ...
                    target_datetime = datetime.combine(
                        target_date, time_obj, tzinfo=TW_TZ
                    )
                    ...
                    scheduled_count += 1
                    self.logger.debug(
                        f"🕐 [_reschedule_push_jobs] 已排程推送任務: {job_id} 於 {target_datetime}"
                    )

                except Exception as e:
                    self.logger.error(
                        f"❌ [_reschedule_push_jobs] 排程推送任務失敗 ({anime_sn}, {video_sn}): {e}"
                    )

            self.logger.info(
                f"🔄 [_reschedule_push_jobs] 推送任務重新排程完成，共 {scheduled_count} 個任務"
            )

        except Exception as e:
            self.logger.error(
                f"❌ [_reschedule_push_jobs] 重新排程推送任務時發生錯誤: {e}",
                exc_info=True,
            )
    ...
    async def _push_anime_task(self, anime_sn: int, video_sn: int):
        ...
        try:
            self.logger.info(
                f"📢 [_push_anime_task] 開始推送動畫: anime_sn={anime_sn}, video_sn={video_sn}"
            )
            ...
            if anime_sn > 0:
                ...
                row = await self.db.fetchone(
                    query, (video_sn, video_sn, anime_sn, anime_sn)
                )
            else:
            ...
            if not row:
                self.logger.warning(
                    f"⚠️ [_push_anime_task] 找不到排程資料: anime_sn={anime_sn}, video_sn={video_sn}"
                )
                return

            week_start_date, day_of_week, scheduled_time = row
            self.logger.info(
                f"📌 [_push_anime_task] 找到排程: week_start={week_start_date}, day={day_of_week}, time={scheduled_time}"
            )
            ...
            success = await self.push_core.send_anime_push(
                scheduled_time, channel_id, day_of_week, week_start_date
            )
            if success:
                self.logger.info(
                    f"✅ [_push_anime_task] 動畫推送成功: anime_sn={anime_sn}, video_sn={video_sn}"
                )
            else:
                self.logger.warning(
                    f"⚠️ [_push_anime_task] 動畫推送未觸發 (可能已推送或無新集數): anime_sn={anime_sn}, video_sn={video_sn}"
                )
        except Exception as e:
            self.logger.error(
                f"❌ [_push_anime_task] 動畫推送任務失敗: {e}", exc_info=True
            )
    ...
    async def _restore_persistent_views(self):
        ...
        try:
            ...
            self.logger.error(
                f"❌ [_restore_persistent_views] 恢復永續視圖失敗: {e}", exc_info=True
            )

    # ==================== 動畫視圖生成方法 ====================

    def generate_anime_view(
        self, episode: Dict[str, Any], video_sn: str
    ...
    async def anime_refresh(self, ctx: commands.Context):
        ...
        try:
            self.logger.info(
                f"🔄 [AnimeTracker.anime_refresh] 手動刷新週表請求 by {ctx.author}"
            )
            result = await self.schedule_tracker.refresh_weekly_schedule()
            ...
            self.logger.error(
                f"❌ [AnimeTracker.anime_refresh] 手動刷新週表失敗: {e}", exc_info=True
            )
            await ctx.followup.send(
                f"❌ 手動刷新週表時發生錯誤: {str(e)}", ephemeral=True
            )
    ...
    async def anime_status(self, ctx: commands.Context):
        ...
        try:
            ...
            # 依賴狀態
            status_lines.append(
                f"🔧 依賴設置: {'✅ 已完成' if self._dependencies_set else '❌ 未完成'}"
            )
            status_lines.append(
                f"🚀 掑程器啟動: {'✅ 已啟動' if self._scheduler_started else '❌ 未啟動'}"
            )
            status_lines.append(
                f"👁️ 視圖恢復: {'✅ 已完成' if self._views_restored else '❌ 未完成'}"
            )
            ...
            await ctx.followup.send(f"❌ 查詢狀態時發生錯誤: {str(e)}", ephemeral=True)
