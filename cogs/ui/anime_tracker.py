# -*- coding: utf-8 -*-
"""
Bahamut 動畫追蹤 Cog - 精準排程推送

核心邏輯：智能排程檢查 → 到達推送時間則推送對應動畫
使用主要資料庫：user_data.db (anime_weekly_schedule 表)
移除輪詢機制，改為智能排程檢查
"""
...
class AnimeTracker(commands.Cog):
    ...
    async def set_dependencies(self, db_path: str = None):
        ...
        self._running = True
        msg = "✅ [AnimeTracker.set_dependencies] 排版推送系統啟動完成 (智能排程檢查)"
        logger.info(msg)
    ...
    async def cog_unload(self):
        """Cog 卸載時清理"""
        self.logger.info("🛑 [AnimeTracker.cog_unload] 正在停止排程檢查...")
        if self._running:
            await self.push_core.stop_polling()
            self._running = False
        self.logger.info("🛑 [AnimeTracker.cog_unload] 排程檢查已停止")
        ...
        try:
            ...
            if self.push_core and self.db:
                ...
                try:
                    ...
                    status_lines.append(f"📊 統計資訊: 無法獲取")
    ...
    async def anime_manual_check(self, ctx: commands.Context):
        ...
        try:
            ...
            # 手動觸發當前時間的推送檢查
            await send_response("🔄 正在手動檢查當前時間的排程...", ephemeral=True)
