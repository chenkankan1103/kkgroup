...
class AnimeTracker(commands.Cog):
    ...
    #     """自動發送週統計 - 每週天 台灣時間 23:00 發送 - DISABLED: Replaced by daily check"""
    #     pass
        now = datetime.now(TW_TZ)
    ...
    async def before_daily_anime_check(self):
        """在第一次循環前等待 bot 就緒"""
        logger.info("🚀 [before_daily_anime_check] 等待 bot 就緒...")
        await self.bot.wait_until_ready()
        logger.info("✅ [before_daily_anime_check] Bot 已就緒，每日檢查任務準備就緒")
