...
class CookieExpireNotifier:
    ...
    async def send_cookie_expired_alert(self, status="EXPIRED", details=None):
        ...
        embed = discord.Embed(
            ...
            description="爬蟲無法正常運行，需要手動介入",
            color=color,
            timestamp=datetime.now(),
        )

        embed.add_field(name="📋 失效狀態", value=f"```{status}```", inline=False)
        ...
        if details:
            embed.add_field(name="📝 詳情", value=f"```{details}```", inline=False)
...
class ThreadsCookieMonitor(commands.Cog):
    ...
    async def check_cookie_status(self, ctx):
        ...
        embed = discord.Embed(
            title=f"📋 Cookie 狀態: {status}", color=color, timestamp=datetime.now()
        )

        embed.add_field(name="📅 年齡", value=f"`{age_days:.1f} 天`", inline=True)
        ...
        try:
            ...
            embed.add_field(
                name="🔐 Cookie 數量", value=f"`{len(cookies)} 個`", inline=True
            )
        except:
            embed.add_field(name="🔐 Cookie 數量", value="`無法讀取`", inline=True)
    ...
    async def update_cookies_manual(self, ctx):
        ...
        embed.add_field(name="步驟 1", value="在本機環境打開終端機", inline=False)
        ...
        embed.add_field(
            name="步驟 3", value="在瀏覽器中登入 Threads (30 秒內)", inline=False
        )
