        ...
        try:
            ...
            for child in self.slot_machine_view.children:
                if isinstance(child, discord.ui.Select):
                    ...
                    isinstance(child, discord.ui.Button) and child.custom_id == "gamble"
                ):
...
class SlotMachineView(discord.ui.View):
    ...
    @discord.ui.button(
        label="下注 50 KKcoin", style=discord.ButtonStyle.primary, emoji="💰"
    )
        ...
        try:
            ...
            try:
                ...
                if not interaction.response.is_done():
                    await interaction.response.send_message(
                        f"❌ 下注處理失敗: {err_text}", ephemeral=True
                    )
                else:
    ...
    @discord.ui.button(
        label="下注 100 KKcoin", style=discord.ButtonStyle.primary, emoji="💎"
    )
    async def bet_100(
    ...
    @discord.ui.button(
        label="下注 200 KKcoin", style=discord.ButtonStyle.primary, emoji="💸"
    )
    async def bet_200(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        label="結束遊戲", style=discord.ButtonStyle.danger, emoji="🚪", row=1
    )
    async def end_game(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        ...
        try:
            ...
            if len(self.history) == 0:
                ...
                if len(self.history) > 0:
                    ...
                    embed.add_field(
                        name="📈 勝率分析", value=f"勝率：{win_rate:.1f}%", inline=False
                    )
...
class PersistentView(PersistentViewBase):
    ...
    async def explore_button(
        ...
        await interaction.response.send_message(
            embed=embed, view=ExploreView(self.cog), ephemeral=True
        )

    @discord.ui.button(
        label="離開", style=discord.ButtonStyle.secondary, custom_id="persistent_exit"
    )
    async def exit_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await interaction.response.send_message(
            "你決定離開這片神秘的黑暗角落。", ephemeral=True
        )
...
class ExploreView(PersistentViewBase):
    ...
    @discord.ui.button(
        ...
        custom_id="persistent_products",
    )
    async def products_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        embed = discord.Embed(
            title="🛒 商品區",
    ...
    @discord.ui.button(
        ...
        custom_id="persistent_rob",
    )
    async def rob_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        label="🎰 拉霸機",
        style=discord.ButtonStyle.secondary,
        custom_id="persistent_gamble",
    )
    async def gamble_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        try:
            await interaction.response.defer(ephemeral=True)
            ...
            message = await interaction.followup.send(
                embed=rules_embed, view=view, ephemeral=True
            )
    ...
    @discord.ui.button(
        ...
        custom_id="persistent_paperdoll",
        emoji="👗",
    )
    async def paperdoll_button(
        ...
        try:
            with open("paperdoll_button_click.log", "a") as f:
                f.write(
                    f"[{ts}] Paperdoll button clicked. Categories count: {len(self.cog.categories)}\n"
                )
                f.write(
                    f"[{ts}] Categories: {self.cog.categories[:5] if self.cog.categories else 'None'}\n"
                )

            if not self.cog.categories:
                await interaction.followup.send(
                    "❌ 衣帽間暫時無法使用。", ephemeral=True
                )
                return

            embed = discord.Embed(
                title="👗 衣帽間", description="選擇要更改的部位。", color=0xFF69B4
            )
...
class RoleShopView(PersistentViewBase):
    ...
    @discord.ui.button(
        label="七彩披風 (500 KKcoin/1天)",
        style=discord.ButtonStyle.primary,
        custom_id="persistent_buy_rainbow",
    )
    async def rainbow_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        item = ROLE_SHOP["七彩披風"]
        await self.cog.handle_role_purchase(
            interaction, "七彩披風", item["price"], item["role_id"], item["duration"]
        )

    @discord.ui.button(
        label="進階組員 (1000 KKcoin/1週)",
        style=discord.ButtonStyle.primary,
        custom_id="persistent_buy_vip",
    )
    async def vip_button(
        ...
        item = ROLE_SHOP["進階組員"]
        await self.cog.handle_role_purchase(
            interaction, "進階組員", item["price"], item["role_id"], item["duration"]
        )

    @discord.ui.button(
        label="返回",
        style=discord.ButtonStyle.secondary,
        custom_id="persistent_role_back",
    )
...
class EquipmentShopView(PersistentViewBase):
    ...
    def update_items(self):
        ...
        for item_name, item_data in items.items():
            ...
            button.callback = (
                lambda i, name=item_name, data=item_data: self.show_item_options(
                    i, name, data
                )
            )
        ...
        back_button = Button(
            label="返回",
            style=discord.ButtonStyle.secondary,
            custom_id="persistent_equipment_back",
        )
    ...
    async def show_item_options(
        self, interaction: discord.Interaction, item_name: str, item_data: dict
    ):
        """顯示商品的詳細選項（預覽、試穿、購買）"""
        await interaction.response.defer(ephemeral=True)
        ...
        embed.add_field(
            name="🏷️ 類別",
            value=self.get_category_name(self.current_category),
            inline=True,
        )
        ...
        view = ItemDetailView(
            self.cog, item_name, item_data, self.current_category, can_afford
        )
...
class ItemDetailView(View):
    ...
    def __init__(
        self, cog, item_name: str, item_data: dict, category: str, can_afford: bool
    ):
        super().__init__(timeout=300)
        self.cog = cog
    ...
    @discord.ui.button(
        label="預覽試穿", style=discord.ButtonStyle.secondary, emoji="🔍"
    )
    async def preview_item(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        await self.cog.handle_equipment_preview(
            interaction, self.item_name, self.item_data, self.category
        )
    ...
    async def buy_item(
        ...
        if not self.can_afford:
            await interaction.response.send_message(
                "❌ 你的 KKcoin 不足，無法購買此商品！", ephemeral=True
            )
            return
        await self.cog.handle_equipment_purchase(
            interaction, self.item_name, self.item_data, self.category
        )
    ...
    async def back_to_shop(
        ...
        await interaction.response.edit_message(
            embed=embed, view=EquipmentShopView(self.cog)
        )
...
class TryOnResultView(discord.ui.View):
    ...
    @discord.ui.button(
        label="喜歡！立即購買", style=discord.ButtonStyle.success, emoji="❤️"
    )
    async def purchase_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
...
class PaperDollPreviewView(discord.ui.View):
    ...
    async def update_preview(self, interaction: discord.Interaction):
        ...
        with open("paperdoll_random.log", "a", encoding="utf-8") as f:
            f.write(
                f"[{ts}] update_preview called, preview_items={self.preview_items}\n"
            )
        # 合併用戶原有裝備和預覽裝備
        ...
        if character_image:
            ...
            files = []
            embed.add_field(
                name="⚠️ 注意", value="預覽圖片生成失敗，請稍後再試", inline=False
            )

        # 嘗試透過 response.edit_message 更新；若已回應則捕捉錯誤然後透過 followup
        try:
            await interaction.response.edit_message(
                embed=embed, view=self, attachments=files
            )
        except Exception:
    ...
    async def select_category(
        ...
        view = EditView(
            ...
            page=0,
            embed=embed,
            original_message=getattr(self, "original_message", interaction.message),
        )
        try:
            await interaction.response.edit_message(embed=embed, view=view)
...
class ProductCategoryView(PersistentViewBase):
    ...
    @discord.ui.button(
        label="購買身份",
        style=discord.ButtonStyle.success,
        custom_id="persistent_buy_roles",
        emoji="👑",
    )
    async def buy_roles_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
    ...
    @discord.ui.button(
        label="種植大麻",
        style=discord.ButtonStyle.success,
        custom_id="persistent_cannabis",
        emoji="🌱",
    )
    async def cannabis_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
        ...
        await interaction.response.send_message(
            embed=embed, view=CannabisMerchantViewV2(self.cog), ephemeral=True
        )

    @discord.ui.button(
        label="⬅️ 返回",
        style=discord.ButtonStyle.secondary,
        custom_id="persistent_back_to_main",
    )
    async def back_button(
        self, interaction: discord.Interaction, button: discord.ui.Button
    ):
