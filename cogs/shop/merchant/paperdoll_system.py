...
class EnhancedPaperDollSystem:
    ...
    def _generate_cache_key(
        self, user_data: dict, category: str = None, item_id: int = None
    ) -> str:
        """生成快取鍵值"""
        ...
        for equipment in equipment_order:
            ...
            if item_value:  # 只在有值時添加
                if field_name == "face" and len(equipment) > 2:
                    ...
                    items.append(
                        {"itemId": item_value, "region": "TWMS", "version": "256"}
                    )
    ...
    async def fetch_try_on_image(
        self, user_data: dict, category: str, item_id: int
    ) -> Optional[discord.File]:
        """獲取試穿圖片"""
                ...
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        if len(image_data) > 100:
                            ...
                            return discord.File(
                                io.BytesIO(image_data), filename="try_on.png"
                            )
                    else:
    ...
    async def create_comparison_embed(
        self,
        ...
        if len(files) == 2:
            ...
            embed.add_field(name="📸 預覽說明", value="顯示試穿後的效果", inline=False)
        else:
            embed.add_field(
                name="⚠️ 提示", value="暫時無法生成預覽圖片，但功能正常", inline=False
            )

        return embed, files

    async def handle_enhanced_try_on(
        self,
        interaction: discord.Interaction,
        item_name: str,
        item_data: dict,
        ...
        await interaction.followup.edit_message(
            message_id=interaction.message.id, embed=loading_embed
        )
        ...
        try:
            ...
            await interaction.followup.edit_message(
                message_id=interaction.message.id, embed=embed, view=view, files=files
            )
            ...
            await interaction.followup.edit_message(
                message_id=interaction.message.id, embed=error_embed, view=view
            )
    ...
    def clear_expired_cache(self):
        ...
        now = datetime.now()
        expired_keys = [
            key for key, expiry_time in self.cache_expiry.items() if now >= expiry_time
        ]
