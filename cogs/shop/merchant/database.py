...
async def update_user_equipment(
    user_id: int, equipment_type: str, item_id: int
) -> bool:
    """更新用戶裝備"""
...
async def add_temp_role(
    user_id: int, role_id: int, guild_id: int, duration_seconds: int
) -> bool:
    """添加臨時角色記錄"""
    ...
    try:
        ...
        set_user_field(
            user_id, "temp_role_info", json.dumps(temp_role_data, ensure_ascii=False)
        )
        return True
    except Exception as e:
        print(f"❌ 添加臨時角色失敗: {e}")
