...
class CannabisFarmingAdapter:
    ...
    async def add_plant(self, user_id: int, plant_data: dict) -> bool:
        ...
        try:
            ...
            def _do_add():
                ...
                # 保存回數據庫
                return self.db.set_user_field(
                    user_id, "cannabis_plants", json.dumps(plants, ensure_ascii=False)
                )
                ...
                for plant in plants_list:
                    if plant.get("id") == plant_id:
                        ...
                        return self.db.set_user_field(
                            user_id,
                            "cannabis_plants",
                            json.dumps(plants_list, ensure_ascii=False),
                        )
    ...
    async def add_inventory(
        self, user_id: int, item_type: str, item_name: str, quantity: int = 1
    ) -> bool:
        """增加庫存"""
        try:
