...
class CannabisBuyView(PersistentViewBase):
    def __init__(self, bot):
        ...
        # 使用全域按鈕系統
        self.add_button(
            label="🌱 購買種子", callback=self.buy_seeds, style="success"
        ).add_button(label="💧 購買肥料", callback=self.buy_fertilizer, style="success")
    ...
    async def buy_seeds(self, interaction: discord.Interaction):
        try:
            ...
            for seed_name, seed_config in CANNABIS_SHOP["種子"].items():
                ...
                embed.add_field(
                    name=f"{seed_config['emoji']} {seed_name}",
                    value=value,
                    inline=False,
                )
    ...
    async def buy_fertilizer(self, interaction: discord.Interaction):
        try:
            ...
            for fertilizer_name, fert_config in CANNABIS_SHOP["肥料"].items():
                ...
                embed.add_field(
                    name=f"{fert_config['emoji']} {fertilizer_name}",
                    value=value,
                    inline=False,
                )
...
class SeedButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            if kkcoin < price:
                await interaction.followup.send(
                    f"❌ KKcoin 不足！需要 {price}，你有 {kkcoin}", ephemeral=True
                )
                return
...
class PlantActionButton(discord.ui.Button):
    def __init__(self, bot, plant, user_id):
        ...
        status = (
            "✅收割" if plant["status"] == "harvested" else f"{plant['progress']:.0f}%"
        )
        super().__init__(
            label=status,
            style=discord.ButtonStyle.success
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            if self.plant["status"] == "harvested":
                ...
                await interaction.followup.send(
                    "選擇肥料施用：", view=view, ephemeral=True
                )
                return
...
class SellCannabisButton(discord.ui.Button):
    def __init__(self, bot, seed_name):
        ...
        self.seed_name = seed_name
        super().__init__(
            label=f"賣 {seed_name}", style=discord.ButtonStyle.danger, emoji="💰"
        )

    async def callback(self, interaction: discord.Interaction):
        try:
            await interaction.response.send_modal(
                SellCannabisModal(self.bot, interaction.user.id, self.seed_name)
            )
        except Exception as e:
            traceback.print_exc()
...
class SellCannabisModal(discord.ui.Modal):
    def __init__(self, bot, user_id, seed_name):
        ...
        self.quantity_input = discord.ui.TextInput(
            label="數量", placeholder="輸入要賣出的數量", required=True
        )
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            # 檢查庫存並出售
            if not await remove_inventory(
                self.user_id, "大麻", self.seed_name, quantity
            ):
