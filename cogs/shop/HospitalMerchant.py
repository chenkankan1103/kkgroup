...
class PersistentStaminaView(PersistentViewBase):
    ...
    async def buy_small(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        await self.merchant_cog.process_purchase(
            interaction, interaction.user.id, "small"
        )

    async def buy_medium(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        await self.merchant_cog.process_purchase(
            interaction, interaction.user.id, "medium"
        )

    async def buy_large(self, interaction: discord.Interaction):
        await interaction.response.defer(ephemeral=True)
        await self.merchant_cog.process_purchase(
            interaction, interaction.user.id, "large"
        )
...
class HospitalMerchant(commands.Cog):
    ...
    async def generate_merchant_message(
        self, user_data_sample: dict = None
    ) -> discord.Embed:
        """使用 AI 生成嘲諷的商人訊息"""
        try:
...
範例風格參考：「看你這狼狽的樣子，肯定是又被打得服服貼貼了。別急，我這邊有最新鮮的進口藥劑，保證讓你起死回生...或至少能走動。怎麼樣，要來點嗎？」"""
    ...
    async def create_merchant_embed(self) -> discord.Embed:
        ...
        # 使用商人圖片作為大圖
        embed.set_image(
            url="https://cdn.discordapp.com/attachments/1275688788806467635/1427730328792989788/image.png"
        )
    ...
    async def setup_merchant_message(self):
        ...
        try:
            ...
            cursor.execute(
                "SELECT config_value FROM merchant_config WHERE config_key = 'message_id'"
            )
            ...
            if result:
                ...
                try:
                    ...
                    cursor.execute(
                        "DELETE FROM merchant_config WHERE config_key = 'message_id'"
                    )
    ...
    async def process_purchase(
        self, interaction: discord.Interaction, user_id: int, product_type: str
    ):
        """處理購買"""
        try:
            ...
            purchase_embed.add_field(
                name="體力恢復", value=f"+{stamina_gain}", inline=True
            )
            purchase_embed.add_field(
                name="剩餘金錢", value=f"{new_kkcoin} KKCoin", inline=True
            )
            purchase_embed.add_field(
                name="當前體力", value=f"{new_stamina}/100", inline=True
            )
            ...
            await interaction.followup.send(
                f"❌ 購買過程中發生錯誤: {e}", ephemeral=True
            )

    async def complete_hospital_recovery(
        ...
        try:
            ...
            if member_role and member_role not in member.roles:
                await member.add_roles(
                    member_role, reason="體力完全恢復，恢復正式成員身分"
                )
            ...
            if channel:
                await channel.send(
                    embed=recovery_embed, content=member.mention, delete_after=None
                )
    ...
    async def on_user_injured(self, user_id: int):
        ...
        try:
            ...
            if channel:
                ...
                await channel.send(
                    embed=personal_embed, content=member.mention, delete_after=None
                )
