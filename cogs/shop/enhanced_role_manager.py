...
class EnhancedRoleExpirationManager(commands.Cog):
    ...
    async def grant_temporary_role(
        ...
        try:
            ...
            if duration_days < 1 or duration_days > 365:
                await interaction.response.send_message(
                    "❌ 持續時間必須在 1-365 天之間！", ephemeral=True
                )
                return
            ...
            if not member:
                await interaction.followup.send(
                    f"❌ {user.mention} 不是此伺服器的成員！", ephemeral=True
                )
                return
            ...
            traceback.print_exc()
            await interaction.followup.send(
                f"❌ 操作失敗：{str(e)[:100]}", ephemeral=True
            )

    @app_commands.command(name="check_my_roles", description="查看你的臨時身分有效期")
    async def check_my_roles(self, interaction: discord.Interaction):
        """查看用戶自己的臨時身分"""
        try:
            await interaction.response.defer(ephemeral=True)

            user_roles = self._get_user_roles(interaction.user.id, interaction.guild.id)

            if not user_roles:
                await interaction.followup.send("你沒有任何臨時身分", ephemeral=True)
                return

            current_time = datetime.now()
            embed = discord.Embed(title="⏱️ 你的臨時身分", color=discord.Color.blue())
    ...
    def _get_user_roles(self, user_id: int, guild_id: int) -> list:
        """獲取用戶的臨時身分"""
        try:
            conn = sqlite3.connect(DB_PATH)
            conn.execute("PRAGMA busy_timeout=30000")  # ✅ 避免多進程鎖定
