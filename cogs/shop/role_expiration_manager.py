    ...
    async def cleanup_expired_roles(self, bot: discord.Client) -> int:
        ...
        for user_id, guild_id, role_id, role_name in expired_roles:
            try:
                ...
                if not member:
                    print(
                        f"[RoleExpiration] ⚠️ 用戶 {user_id} 不在伺服器 {guild_id}，標記為已處理"
                    )
                    self.mark_as_removed(user_id, guild_id, role_id)
                    removed_count += 1
                    continue
                ...
                if not role:
                    print(
                        f"[RoleExpiration] ⚠️ 角色 {role_id} ({role_name}) 不存在，標記為已處理"
                    )
                    self.mark_as_removed(user_id, guild_id, role_id)
                    removed_count += 1
                    continue
                ...
                if role in member.roles:
                    ...
                    print(
                        f"[RoleExpiration] ✅ 已移除 {member.display_name} 的 {role_name} 身分"
                    )
                    removed_count += 1
                else:
                    # 角色已不在成員上，直接標記為已處理
                    print(
                        f"[RoleExpiration] ℹ️ {member.display_name} 已不擁有 {role_name} 身分"
                    )
                ...
                print(
                    f"[RoleExpiration] ❌ 移除失敗 (用戶 {user_id}, 角色 {role_id}): {e}"
                )

        if removed_count > 0:
            print(
                f"[RoleExpiration] ✅ 本次清理完成：移除了 {removed_count} 個過期角色"
            )
