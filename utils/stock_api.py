...
async def fetch_chart(
    ...
    try:
        ...
        if not hist_data or not hist_data["prices"]:
            logger.warning(
                f"⚠️ {symbol} 在 period={period} interval={interval} 下無歷史數據"
            )
            return None
        ...
        chart_config = {
            "type": "line",
            "data": {"labels": sampled_dates, "datasets": datasets},
            "options": {
                "responsive": True,
                "maintainAspectRatio": True,
        ...
        print(
            f"[CHART_API] {symbol} 正在生成圖表 (period={period}, interval={interval}, 數據點={len(sampled_prices)})",
            flush=True,
        )
        logger.info(
