...
async def test_debug():
    ...
    # 這裡模擬 pytest fixture 行為：先 freeze 到 00:00
    freezer = TimeFreezer()
    ...
    # ===== Fixture setup 執行 cog_load（在 00:00）=====
    ...
    print(f'After cog_load at 00:00, frozen time: {now}')
    ...
    # ===== 現在測試 freeze 到 01:00（模擬測試呼叫 frozen_time.freeze）=====
    target_time = datetime(2026, 8, 10, 1, 0, 0, tzinfo=TW_TZ)
    freezer.freeze(target_time)

    now = freezer.frozen_dt
    print(f'After re-freeze to 01:00, frozen time: {now}')
    ...
    # Check DB before UPDATE
    conn = get_db_conn(cog.db.db_path)
    cursor = conn.cursor()
    cursor.execute(f'''
    ...
    row = cursor.fetchone()
    conn.close()
    print(f'DB BEFORE UPDATE: animeData={row[0]}, pushed={row[1]}')

    # Test UPDATE (like the test does)
    conn = get_db_conn(cog.db.db_path)
    cursor = conn.cursor()
    cursor.execute(f'''
