...
async def test_debug():
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT weekStartDate, dayOfWeek, scheduledTime, animeData, pushed FROM anime_weekly_schedule"
    )
    ...
    for row in rows:
        print(
            f"  weekStartDate={row[0]}, dayOfWeek={row[1]}, scheduledTime={row[2]}, pushed={row[4]}"
        )
