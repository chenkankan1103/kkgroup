        ...
        try:
            ...
            if level < 5:
                ...
                if can_level_up:
                    ...
                    current_days_text = f"{info['current_days']}/{info['required_days']} ??
                    weeks_text = f"({current_weeks}/{required_weeks} ??"
                    xp_text = f"{info['current_xp']:,}/{info['required_xp']:,} XP"
                    days_met_text = "?????????? if info['days_met'] else "???????????"
                    xp_met_text = "?????????? if info['xp_met'] else "???????????"
                    
                    progress_text = (
                        f"{days_status} **??????**??current_days_text} {weeks_text}\n"
                        f"{xp_status} **??????**??xp_text}\n\n"
                        f"{days_met_text}\n"
                        f"{xp_met_text}"
                    )
