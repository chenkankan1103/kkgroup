...
def _log_operation(
    user_id: Optional[int], action: str, details: str = "", success: bool = True
):
    """記錄敏感操作（用於審計）"""
...
def _resolve_user_id(
    user_id: str, caller_id: Optional[int], context: str
    ...
    if not user_id or not user_id.isdigit():
        if caller_id:
            ...
            return (
                None,
                f"❌ 無法確定要查詢哪個用戶的{context}。請提供用戶 ID 或 @tag 我來查詢你的{context}。",
            )
    return (user_id, None)
...
def get_kkcoin_balance(user_id: str = "", *, caller_id: Optional[int] = None) -> str:
    ...
    try:
        ...
        result = (
            f"用戶 {user_id} — KK幣：{kkcoin:.1f} KKC，數位美金：${digital_usd:.2f}"
        )
...
@register_tool(
    ...
    parameters={
        "type": "OBJECT",
        "properties": {"user_id": {"type": "STRING", "description": "Discord 用戶 ID"}},
        "required": ["user_id"],
    },
)
...
def get_top_kkcoin_leaderboard(
    top_n: int = 10, *, caller_id: Optional[int] = None
) -> str:
    """
    查詢 KK幣排行榜前 N 名。
    ...
    try:
        ...
        if not all_users:
            return "暫無用戶數據。"
        sorted_users = sorted(
            all_users, key=lambda u: float(u.get("kkcoin", 0) or 0), reverse=True
        )[:top_n]
        medals = ["🥇", "🥈", "🥉"]
...
def get_bot_status(*, caller_id: Optional[int] = None) -> str:
    ...
    uptime_file = os.path.join(
        os.path.dirname(os.path.abspath(__file__)), "data", "bot_start_time.txt"
    )
    start_time = "未知"
    try:
...
def trigger_git_push(commit_message: str, *, caller_id: Optional[int] = None) -> str:
    ...
    try:
        ...
        # 4. git push
        res = subprocess.run(
            ["git", "push"], cwd=repo_path, capture_output=True, text=True, timeout=60
        )
        if res.returncode != 0:
...
@register_tool(
    ...
    parameters={
        ...
        "properties": {
            ...
            "item_name": {"type": "STRING", "description": "裝備名稱"},
            "power": {"type": "INTEGER", "description": "裝備力量值（可選，預設 0）"},
        },
...
def set_maplestory_equipment_item(
    ...
    *,
    ...
    try:
        ...
        # 儲存回資料庫
        set_user_field(
            user_id, "maplestory_equipment", json.dumps(equipped, ensure_ascii=False)
        )
...
def remove_maplestory_equipment_item(
    user_id: str, slot: str, *, caller_id: Optional[int] = None
) -> str:
    """
    卸下指定位置的裝備
...
@_require_leader
def run_terminal(
    command: str, timeout_sec: int = 30, *, caller_id: Optional[int] = None
) -> str:
    """
    ...
    for service in services_to_query:
        ...
        if filter_keyword:
            commands_to_try.append(
                f"sudo journalctl -u {service_unit} -n {lines} --no-pager | grep -iE '{filter_keyword}'"
            )
        else:
            commands_to_try.append(
                f"sudo journalctl -u {service_unit} -n {lines} --no-pager"
            )

        # 【降級】不用 sudo 的 journalctl
        if filter_keyword:
            commands_to_try.append(
                f"journalctl -u {service_unit} -n {lines} --no-pager | grep -iE '{filter_keyword}'"
            )
        else:
            commands_to_try.append(
                f"journalctl -u {service_unit} -n {lines} --no-pager"
            )
        ...
        if not success:
            # 所有命令都失敗了
            section.append("❌ 無法查詢日誌（journalctl 命令不可用或無權限）")
...
def read_project_file(file_path: str, *, caller_id: Optional[int] = None) -> str:
    ...
    try:
        ...
        if not str(full_path).startswith(str(pathlib.Path(project_root).resolve())):
            return "❌ 安全檢查失敗：路徑超出專案目錄。只允許讀取專案內的文件。"
        ...
        if os.sep not in file_path and "/" not in file_path:
            ...
            matches = list(
                project_path.rglob(
                    f"{file_path}" if file_path.endswith(".py") else f"{file_path}.py"
                )
            )
            ...
            if matches:
                if len(matches) == 1:
                    ...
                    options = "\n".join(
                        [f"  • {p.replace(os.sep, '/')}" for p in relative_paths]
                    )
            ...
            if content_matches:
                if len(content_matches) == 1:
                    ...
                    # 找到多個，列出選項
                    relative_paths = [
                        str(m.relative_to(project_root)) for m in content_matches
                    ]
                    options = "\n".join(
                        [f"  • {p.replace(os.sep, '/')}" for p in relative_paths]
                    )
...
def get_git_status(*, caller_id: Optional[int] = None) -> str:
    ...
    try:
        ...
        current_branch = (
            branch_result.stdout.strip() if branch_result.returncode == 0 else "未知"
        )
        ...
        last_commit = (
            log_result.stdout.strip() if log_result.returncode == 0 else "未知"
        )
        ...
        remote_status = (
            status_vs_remote.stdout.strip()
            if status_vs_remote.returncode == 0
            else "未知"
        )
...
def get_operation_log(limit: int = 10, *, caller_id: Optional[int] = None) -> str:
    ...
    for log in reversed(recent_logs):
        ...
        lines.append(
            f"  {status_icon} {log['timestamp']} | {user} | {log['action']}{details}"
        )
...
def get_gemini_tools_spec() -> List[Dict]:
    ...
    """
    return [{"functionDeclarations": [v["spec"] for v in _TOOL_REGISTRY.values()]}]
...
def smart_search_code(
    ...
    *,
    ...
    try:
            ...
            try:
                ...
                for line_num, line in enumerate(lines, 1):
                    ...
                    if "#" in line and line.find("#") < line.find(
                        search_pattern if not is_regex else "match"
                    ):
                        comment_matches.append(match_info)
                    elif "=" in line and any(
                        x in line for x in ["const", "MAX", "LIMIT", "= "]
                    ):
        ...
        if exact_matches:
            report_lines.append(
                f"✅ 【確實需要改】常數/配置定義（{len(exact_matches)} 處）："
            )
            for m in exact_matches[:5]:
                report_lines.append(
                    f"   {m['file']}:{m['line']} → {m['content'][:60].strip()}"
                )
            ...
            "replacements": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "search": {"type": "STRING", "description": "要搜尋的文字"},
                        "replace": {"type": "STRING", "description": "要替換成的文字"},
                    },
                    "required": ["file", "search", "replace"],
                },
                "description": "批量修改清單（數組）",
            },
            "commit_message": {"type": "STRING", "description": "Git 提交訊息"},
        },
        "required": ["replacements", "commit_message"],
    },
)
@_require_leader
def batch_replace_code(
    replacements: List[Dict], commit_message: str, *, caller_id: Optional[int] = None
) -> str:
    """
    批量修改代碼 - 一次提交多個文件修改。
    ...
    try:
        ...
        for i, v_result in enumerate(validation_results, 1):
            report_lines.append(
                f"  {i}. {v_result['file']} - {v_result['count']} 處修改"
            )
...
async def search_web(
    query: str, max_results: int = 5, *, caller_id: Optional[int] = None
) -> str:
    """
    網頁搜尋工具。
...
def diagnose_problem(
    problem_description: str, error_output: str = "", *, caller_id: Optional[int] = None
) -> str:
    """
    智能問題診斷 - 自動分析問題根源。
    ...
    try:
        ...
        if related_files:
            suggestions.append(
                f"📄 相關文件：{', '.join([f.name for f in related_files[:3]])}"
            )

        if error_type == "語法錯誤":
            suggestions.append(
                "✅ 建議：1) 檢查最近修改的代碼 2) 運行 Python 語法檢查 3) 查看錯誤行號"
            )
        elif error_type == "資料訪問錯誤":
            suggestions.append(
                "✅ 建議：1) 檢查字典/列表鍵是否存在 2) 添加數據驗證 3) 使用 .get() 方法"
            )
        elif error_type == "類型錯誤":
            suggestions.append(
                "✅ 建議：1) 檢查函數參數類型 2) 添加類型提示（Type Hints）"
            )
        elif error_type == "權限錯誤":
            suggestions.append(
                "✅ 建議：1) 檢查 LEADER_ID 設定 2) 驗證 caller_id 是否正確"
            )
        elif error_type == "運行時例外":
            suggestions.append(
                "✅ 建議：1) 查看日誌文件 2) 添加更詳細的錯誤捕獲 3) 檢查外部依賴（DB、API）"
            )
        ...
        if related_files:
            for f in related_files[:5]:
                report_lines.append(
                    f"  • {str(f.relative_to(project_root)).replace(os.sep, '/')}"
                )
        else:
        ...
        report_lines.append("\n💡 下一步：")
        if related_files:
            report_lines.append("  1) 使用 smart_search_code 搜尋相關代碼")
            report_lines.append("  2) 分析代碼邏輯")
            report_lines.append("  3) 使用 batch_replace_code 修復問題")
        else:
            report_lines.append("  1) 提供更詳細的錯誤訊息")
            report_lines.append("  2) 查看相關日誌文件")
...
@register_tool(
    ...
    parameters={
        "type": "OBJECT",
        "properties": {
            "file_path": {"type": "STRING", "description": "要修復的文件路徑"},
            "problem_pattern": {
                "type": "STRING",
...
def generate_fix_suggestion(
    file_path: str, problem_pattern: str, *, caller_id: Optional[int] = None
) -> str:
    """
    生成智能修復建議 - 完整的解決方案。
...
sanitized = escape_string(user_input)""",
        ...
        suggestion = suggestions.get(
            problem_pattern, suggestions["missing-error-check"]
        )
        ...
        report_lines = [
            ...
            f"  {suggestion['test']}",
            "\n【回退計劃】：",
            "  如修復後出現問題，使用 git revert <commit_hash> 迅速回退",
            "\n💡 後續：",
            "  1) 修改代碼",
            "  2) 運行測試",
            "  3) 使用 batch_replace_code 提交修改",
        ]
    ...
    *,
    ...
    try:
        ...
        if workflow_type == "update-constant":
            ...
            old_val, new_val = (
                (target.split("→")[0].strip(), target.split("→")[1].strip())
                if "→" in target
                else (target, details)
            )
            ...
            report_lines.extend(
                [
                    ...
                    "   → 定位問題根源和相關文件",
                    "\n2️⃣ 搜尋相關代碼...",
                    "   → 使用 smart_search_code 精準搜尋",
                    "   → 分析問題模式",
                    "\n3️⃣ 生成修復建議...",
                    "   → 使用 generate_fix_suggestion",
                    "   → 獲得完整修復方案",
                    "\n4️⃣ 實施修復...",
                    "   → 根據建議修改代碼",
                    "   → 使用 batch_replace_code 提交",
                    "\n5️⃣ 驗證修復...",
                    "   → 測試修復結果",
                    "   → 檢查是否引入新問題",
                ]
        ...
        report_lines.extend(
            [
                "\n【後續行動】：",
                "✅ 所有步驟已列出",
                "⚠️ 敏感操作（如 batch_replace_code）需要人工確認",
                "💡 建議按序執行上述步驟，每步後檢查結果",
...
@_require_leader
def smart_config_modifier(
    user_command: str, affected_system: str, *, caller_id: Optional[int] = None
) -> str:
    """
        ...
        for pattern, _ in value_patterns:
            ...
            if match:
                ...
                if len(groups) == 2 and groups[1] is not None:  # 從 X 改為 Y
                    ...
                    analysis["value_changes"].append(
                        {"new_value": new_val, "change_type": "simple_change"}
                    )
            ...
            try:
                ...
                if file_matches:
                    ...
                    analysis["affected_files"].append(
                        {"path": relative_path, "matches": file_matches}
                    )
            except:
                pass
        ...
        if analysis["affected_files"]:
            ...
            for file_info in analysis["affected_files"]:
                ...
                for idx, match in enumerate(file_info["matches"][:3], 1):
                    report_lines.append(
                        f"      {idx}. 第 {match['line']} 行：{match['content'][:70]}"
                    )
                if len(file_info["matches"]) > 3:
                    report_lines.append(
                        f"      ... 還有 {len(file_info['matches']) - 3} 處"
                    )
        else:
            report_lines.append("\n⚠️ 【警告】：未找到相關配置文件！")
            report_lines.append("   💡 請檢查：")
            report_lines.append(f"      1) 系統名稱 '{affected_system}' 是否正確")
            report_lines.append(
                f"      2) 配置文件是否存在：{system_config.get('config_file', 'unknown')}"
            )

        # 生成修改建議
        report_lines.append("\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━")
        report_lines.append("\n✅ 【下一步操作】：")
        report_lines.append("   1️⃣ 使用 read_project_file 確認現有值")
        report_lines.append("   2️⃣ 使用 write_project_file 進行修改")
        report_lines.append("   3️⃣ 使用 trigger_git_push 提交變更")
...
def analyze_code_changes(
    change_description: str, search_keyword: str, *, caller_id: Optional[int] = None
) -> str:
    """
    ...
    try:
        ...
        report_lines.append("\n✅ 分析完成！請根據上述位置逐一檢查和修改。")
        report_lines.append(
            "💡 建議：先用 read_project_file 確認各檔案內容，再用 write_project_file 修改。"
        )
...
if __name__ == "__main__":
    ...
    print("嘗試非法訪問：")
    print(
        dispatch_tool("trigger_git_push", {"commit_message": "test"}, caller_id=99999)
    )
    print("\n查看日誌：")
    print(
        dispatch_tool(
            "get_operation_log",
            {"limit": 5},
            caller_id=int(os.getenv("LEADER_DISCORD_ID", "0")),
        )
    )
