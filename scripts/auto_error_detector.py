...
class AutoErrorDetector:
    def __init__(self):
        ...
        self.webhook_url = os.getenv("DISCORD_WEBHOOK_URL") or os.getenv(
            "DISCORD_WEBHOOK"
        )
        ...
        self.max_log_age_seconds = int(
            os.getenv("AUTO_DEBUG_MAX_LOG_AGE_SECONDS", "600")
        )
        self.error_patterns = {
    ...
    def _is_benign_unknown_interaction(self, lines, index):
        ...
        if re.search(
            r"Unknown interaction|error code:\s*10062", current_line, re.IGNORECASE
        ):
            return True
        ...
        return bool(
            re.search(r"Unknown interaction|error code:\s*10062", window, re.IGNORECASE)
        )
    ...
    def _read_journal_lines(self, service_name):
        ...
        if result.returncode != 0:
            raise RuntimeError(
                result.stderr.strip() or f"journalctl failed for {service_name}"
            )
        ...
        try:
            for service_name in self.journal_services:
                try:
                    ...
                    errors_found.extend(
                        self._collect_errors_from_lines(lines, service_name)
                    )
            ...
            for log_file in log_files:
                ...
                try:
                    ...
                    errors_found.extend(
                        self._collect_errors_from_lines(lines, log_file)
                    )
                except Exception as e:
        ...
        try:
            ...
            webhook_data = {
                ...
                "embeds": [
                    {
                        "title": "檢測到系統錯誤",
                        "description": "流程：分析問題 -> debug -> push",
                        "color": 0x57F287 if dispatch_success else 0xED4245,
                        "fields": [
                            {
                                "name": "🔍 錯誤類型",
                                "value": error_data.get("type", "未知"),
                                "inline": True,
                            },
                            {
                                "name": "📁 錯誤文件",
                                "value": error_data.get("file", "未知"),
                                "inline": True,
                            },
                            {
                                "name": "⏰ 檢測時間",
...
async def main():
    ...
    run_once = os.getenv("AUTO_ERROR_DETECTOR_RUN_ONCE", "").strip().lower() in (
        "1",
        "true",
        "yes",
    )
