...
def get_systemctl_statuses() -> Dict[str, str]:
    ...
    for service_name in SERVICE_NAMES:
        statuses[service_name] = (
            run_command(["systemctl", "is-active", service_name]) or "unknown"
        )
    return statuses
...
def derive_expansion_suggestions() -> List[str]:
    ...
    if (
        ...
        and "# ==================== 知識庫管理指令 ===================="
        in memory_manager.read_text(encoding="utf-8")
    ):
        suggestions.append(
            "`cogs/common/memory_manager.py` 仍是骨架，下一步可補管理員 slash commands 管理知識庫。"
        )

    api_index = PROJECT_ROOT / "api_index.json"
    if (
        api_index.exists()
        and "knowledge" not in api_index.read_text(encoding="utf-8").lower()
    ):
        suggestions.append(
            "`api_index.json` 尚未列出 knowledge search endpoint，可補 `/api/knowledge/search` 供外部查詢。"
        )
    ...
    kb_pipeline_doc = (
        ...
        / "_wiki"
        / "concepts"
        / "ai-memory-and-vm-knowledge-pipeline.md"
    )
    if not kb_pipeline_doc.exists():
        suggestions.append(
            "缺少 AI 記憶管線說明頁，建議把掃描與 ingest 流程寫進 knowledge wiki。"
        )
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output", default=str(DEFAULT_OUTPUT), help="輸出 Markdown 路徑"
    )
