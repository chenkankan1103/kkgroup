...
log_path = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "webhook_logger.log"
)
...
# bots_info 文件路徑
bots_info_file = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "bots_info.json"
)
...
async def update_bot_info(
    bot_type: str, startup_time: str, commands: list, extensions: list
):
    global bots_info
    ...
    if bot_type in bots_info:
        ...
        log_webhook(
            f"✅ {bot_type.upper()} 資訊已更新 - 舊時間: {old_time}, 新時間: {startup_time}, 指令: {len(commands)}, 擴展: {len(extensions)}"
        )
        print(f"[DEBUG] {bot_type} bots_info 更新成功 - 時間: {startup_time}")
    else:
        log_webhook(
            f"❌ {bot_type} 不在 bots_info 中 (可用的: {list(bots_info.keys())})"
        )
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    ...
                    try:
                        ...
                        log_webhook(
                            f"❌ Webhook 失敗 (HTTP {resp.status}): {resp_text[:200]}"
                        )
                    except:
...
async def send_or_update_startup_info(bot_type: str = None):
    ...
    try:
        ...
        bots_info = load_bots_info()
        log_webhook(
            f"📂 已重新加載 bots_info: bot={bots_info['bot']['啟動時間']}, shopbot={bots_info['shopbot']['啟動時間']}, uibot={bots_info['uibot']['啟動時間']}"
        )
        ...
        try:
            ...
            while True:
                ...
                all_started = all(
                    bots_info[bot]["啟動時間"] for bot in ["bot", "shopbot", "uibot"]
                )

                # 條件 1: 所有機器人都啟動且滿足最小等待時間
                if all_started and elapsed >= min_wait:
                    log_webhook(
                        f"✅ 所有機器人已啟動 ({int(elapsed)} 秒)，準備發送訊息"
                    )
                    break
                ...
                if elapsed >= max_wait:
                    if all_started:
                        ...
                        missing_bots = [
                            bot
                            for bot in ["bot", "shopbot", "uibot"]
                            if not bots_info[bot]["啟動時間"]
                        ]
                        log_webhook(
                            f"🔴 異常：機器人未在 {max_wait} 秒內啟動 {missing_bots}，強制發送訊息"
                        )
                    break
