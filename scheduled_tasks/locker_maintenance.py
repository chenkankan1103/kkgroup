...
class LockerMaintenanceCog(commands.Cog):
    ...
    async def weekly_locker_check(self):
        ...
        try:
            ...
            for guild in self.bot.guilds:
                ...
                if cleaned > 0:
                    add_log(
                        "bot", f"✅ 在伺服器 {guild.name} 中清理了 {cleaned} 項孤立數據"
                    )
    ...
    async def cleanup_orphaned_data(self, valid_members: set) -> int:
        ...
        try:
            ...
            try:
                c.execute(
                    "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='cannabis_plants'"
                )
                if c.fetchone()[0]:
                    c.execute(
                        "SELECT id, user_id FROM cannabis_plants WHERE user_id NOT IN (SELECT ? FROM (VALUES (?)) AS t(id))"
                    )

                    # 改用 in-memory 檢查
                    c.execute("SELECT id, user_id FROM cannabis_plants")
                    orphaned_plants = [
                        pid for pid, uid in c.fetchall() if uid not in valid_members
                    ]

                    for plant_id in orphaned_plants:
                        c.execute(
                            "DELETE FROM cannabis_plants WHERE id = ?", (plant_id,)
                        )
                        cleaned_count += 1
            ...
            try:
                c.execute(
                    "SELECT COUNT(*) FROM sqlite_master WHERE type='table' AND name='cannabis_inventory'"
                )
                if c.fetchone()[0]:
                    c.execute("SELECT id, user_id FROM cannabis_inventory")
                    orphaned_inventory = [
                        iid for iid, uid in c.fetchall() if uid not in valid_members
                    ]

                    for inv_id in orphaned_inventory:
                        c.execute(
                            "DELETE FROM cannabis_inventory WHERE id = ?", (inv_id,)
                        )
                        cleaned_count += 1
    ...
    @app_commands.command(
        name="locker_health_check", description="🔍 手動檢查置物櫃健康狀態"
    )
    ...
    async def check_locker_health_slash(self, interaction: discord.Interaction):
        ...
        try:
            ...
            if not Path(DB_PATH).exists():
                embed = discord.Embed(
                    title="❌ 置物櫃數據庫不存在", color=discord.Color.red()
                )
            ...
            await interaction.followup.send(
                f"❌ 檢查失敗: {str(e)[:100]}", ephemeral=True
            )
            traceback.print_exc()
