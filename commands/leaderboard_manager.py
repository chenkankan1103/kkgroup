...
# ======================================================================
# QuickChart API - 生成圖表 URL（節省流量實驗）
# ======================================================================

def generate_quickchart_leaderboard_url(members_data, max_visible=15):
    """
    使用 QuickChart API 生成排行榜柱狀圖
    
    優點：
    - 無需本地 PNG 生成，直接返回 URL
    - URL 可直接嵌入 Discord，無需文件上傳
    - 減少 VM 頻寬（無需上傳到 Discord CDN）
    
    缺點：
    - 依賴外部服務 (quickchart.io)
    - 免費版本有速率限制（可能需要 API key）
    - 無法包含頭像等複雜元素
    
    參數:
    - members_data: [(member, kkcoin, digital_usd), ...]
    - max_visible: 最多顯示的排行榜行數
    """
    
    # 限制顯示數量
    members_data = members_data[:max_visible]
    
    if not members_data:
        return "https://quickchart.io/chart?c=" + urllib.parse.quote('{"type":"bar","data":{"labels":[],"datasets":[]}}')
    ...
    for i, member_data in enumerate(members_data):
        ...
        # 簡化名字（最多 12 個字+排名）
        display_name = member.display_name[:12] if hasattr(member, 'display_name') else f"Player {i+1}"
        labels.append(f"{i+1}. {display_name}")
        
        kkcoin_values.append(float(kkcoin or 0))
        dusd_values.append(float(digital_usd or 0))
            ...
            "datasets": [
                {
                    "label": "KK幣",
                    "data": kkcoin_values,
                    "backgroundColor": "rgba(76, 175, 80, 0.8)",  # 綠色
                    "borderColor": "rgba(76, 175, 80, 1)",
                    "borderWidth": 1
                },
                {
                    "label": "D-USD (×35)",
                    "data": [v * dynamic_rate for v in dusd_values],
                    "backgroundColor": "rgba(33, 150, 243, 0.8)",  # 藍色
...
def generate_quickchart_simple_bar(members_data, max_visible=15):
    """
    更簡單的迷你版本 - 只顯示總資產
    
    這個版本會更精簡，容易產生更短的 URL
    """
    members_data = members_data[:max_visible]
    ...
    for i, member_data in enumerate(members_data):
        ...
        display_name = member.display_name[:10] if hasattr(member, 'display_name') else f"P{i+1}"
        labels.append(f"{i+1}.{display_name}")
        
        total = float(kkcoin or 0) + float(digital_usd or 0) * dynamic_rate
        total_assets.append(int(total))
