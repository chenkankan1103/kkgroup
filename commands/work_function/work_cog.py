...
class CheckInButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        try:
            ...
            embed, updated_user = await process_checkin(interaction.user.id, interaction.user)
            
            if embed and updated_user:
                work_view = WorkActionView(updated_user)
                await interaction.followup.send(embed=embed, view=work_view, ephemeral=True)
            else:
                await interaction.followup.send("❌ 處理打卡失敗，請稍後再試或聯絡管理員", ephemeral=True)

        except Exception as e:
            traceback.print_exc()
            try:
                await interaction.followup.send("❌ 發生錯誤，請稍後再試或聯絡管理員。", ephemeral=True)
            except:
                pass
    ...
    async def work(self, interaction: discord.Interaction, action: str = "deploy"):
        try:
            ...
            if action == "deploy":
                ...
                embed = discord.Embed(
                    title="👷‍♀️【KK園區值勤系統】",
                    description="歡迎來到詐騙園區！請選擇今日行動：",
                    color=0x2f3136
                )
                embed.add_field(name="打卡上班", value="獲得基本薪資並維持連續出勤", inline=True)
                embed.add_field(name="休息一天", value="重置連續出勤天數，但不會獲得薪資", inline=True)
                embed.set_footer(text="⚠️ 連續出勤可提升職位等級，獲得更高薪資")
                ...
                # 查看個人狀態
                from .work_system import create_work_embed
                user = get_user(interaction.user.id)
                embed = create_work_embed(user, interaction.user)
                await interaction.followup.send(embed=embed, ephemeral=True)
            
            elif action == "test_intro":
                # 測試介紹論壇檢查功能
                await self.test_user_introduction(interaction)
            
        except Exception as e:
            traceback.print_exc()
            await interaction.followup.send("❌ 執行指令時發生錯誤，請稍後再試", ephemeral=True)
