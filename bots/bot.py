...
async def _sync_app_commands_with_entry_point_recovery() -> list:
    ...
    try:
        ...
        deleted = await asyncio.to_thread(
            _delete_stale_entry_point_commands_sync, application_id
        )
        if deleted > 0:
            file_log(
                f"[SYNC] 已刪除 {deleted} 個殘留 Entry Point commands，重新同步 slash commands"
            )
        else:
            file_log(
                "[SYNC] 偵測到 50240，但未找到殘留 Entry Point command；仍重試一次同步"
            )
...
async def find_and_load_extensions(base_path, package_prefix="", bot_client=None):
    ...
    excluded_modules = {
        ...
        "ai_client_liteLLM",  # AI 客戶端工具模組，不是 Cog
    ...
    for item in sorted(os.listdir(base_path)):
        ...
        if os.path.isdir(item_path) and os.path.exists(
            ...
            sub_extensions = await find_and_load_extensions(
                item_path, sub_package, bot_client
            )
...
async def cleanup_expired_roles_loop():
    ...
    try:
        from cogs.shop.role_expiration_manager import (
            get_manager as get_expiration_manager,
        )
...
async def _check_ready_timeout():
    ...
    for i in range(10):
        ...
        file_log(
            f"[READY_MONITOR] {i+1}s - ready={client.is_ready()} - on_ready_called={_on_ready_called}"
        )
...
async def on_voice_state_update(member, before, after):
    print(
        f"[global] voice_state_update member={member.id} before={getattr(before.channel, 'id', None)} after={getattr(after.channel, 'id', None)}",
        flush=True,
    )
...
async def on_disconnect():
    ...
    file_log(
        f"[{timestamp}] === ON_DISCONNECT CALLED === (Memory: {_get_memory_usage()})"
    )
...
async def on_ready():
    ...
    print(
        "[bot] on_ready triggered, guilds:",
        [(g.id, g.name) for g in client.guilds],
        flush=True,
    )
    ...
    try:
        ...
        if not _on_ready_called:
            ...
            try:
                ...
                file_log(
                    f"[SETUP] setup_modules() completed, loaded {len(loaded_extensions)} extensions"
                )
            except Exception as e:
    ...
    try:
        while True:
            try:
                ...
                timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
                error_msg = (
                    f"[{timestamp}] [ERROR] Run failed (attempt {reconnect_count}): {e}"
                )
                file_log(error_msg)
                print(error_msg)
