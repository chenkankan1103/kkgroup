...
class ScamParkEvents(commands.Cog):
    ...
    @tasks.loop(minutes=20)  # 每20分鐘檢查一次（更頻繁的檢查）
    async def random_event_trigger(self):
        ...
        try:
            ...
            for user_id, thread_id, kkcoin, level, hp, stamina in all_users:
                ...
                # 基礎觸發機率（提高到12%，因為檢查頻率較高）
                base_chance = 0.12 * bell_curve_factor
                
                # KKCoin加成（有錢的人更容易被盯上）
                wealth_bonus = min(0.18, kkcoin / 1500 * 0.1)
                
                # 超過最大間隔後，強制增加機率
                if time_since_last > self.max_event_interval:
                    overtime_bonus = 0.5  # 50%額外機率
                else:
                    overtime_bonus = 0
                ...
                # Debug 日誌（可選）
                if random.random() < 0.05:  # 只記錄5%的檢查，避免日誌過多
...
要讓人感受到詐騙園區的壓迫感和絕望氛圍。"""
    ...
    async def event_major_confiscation(self, member, thread, kkcoin, level, hp, stamina):
        ...
        message = await thread.send(content=member.mention, embed=embed)
