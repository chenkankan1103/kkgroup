...
class ScamParkEvents(commands.Cog):
    def __init__(self, bot):
        ...
        # 事件設定
        self.event_cooldown = {}  # 使用者事件冷卻
        self.min_event_interval = 10800  # 最少3小時 (10800秒)
    ...
    async def random_event_trigger(self):
        ...
        try:
            ...
            cursor.execute('''
                ...
                FROM users 
                WHERE thread_id != 0 AND kkcoin > 0
                ORDER BY kkcoin DESC
            ''')
            rich_users = cursor.fetchall()
            
            # 也獲取沒有KKCoin的使用者（作為備選）
            cursor.execute('''
                SELECT user_id, thread_id, kkcoin, level, hp, stamina 
                FROM users 
                WHERE thread_id != 0 AND kkcoin <= 0
            ''')
            poor_users = cursor.fetchall()
            conn.close()

            current_time = datetime.datetime.now().timestamp()

            # 優先處理有錢的使用者（80%機率）
            target_users = rich_users if random.random() < 0.8 and rich_users else poor_users + rich_users
            
            for user_id, thread_id, kkcoin, level, hp, stamina in target_users:
                # 檢查冷卻時間
                ...
                # KKCoin越多，觸發機率越高（最高+30%）
                wealth_bonus = min(0.3, kkcoin / 1000 * 0.1)
                
                # 距離上次事件越久，觸發機率越高
                time_bonus = min(0.3, time_since_last / self.max_event_interval * 0.3)
                
                trigger_chance = base_chance + wealth_bonus + time_bonus
                ...
                if random.random() < trigger_chance:
                    ...
                    if thread and member:
                        # 第一次事件時自動tag並提示追蹤
                        is_first_event = user_id not in self.event_cooldown
                        
                        await self.trigger_random_event(member, thread, kkcoin, level, hp, stamina, is_first_event)
    ...
    async def generate_pollinations_image(self, prompt: str, is_negative_event: bool = True) -> str:
        ...
        try:
            ...
            if is_negative_event:
                style_keywords = "dark, oppressive, dystopian, surveillance, prison atmosphere"
            else:
                style_keywords = "hopeful, subtle lighting, realistic"
            
            # 完整的提示詞（英文效果較好）
            full_prompt = f"{prompt}, {style_keywords}, digital art, high quality"
            
            # URL 編碼
            encoded_prompt = urllib.parse.quote(full_prompt)
            
            # Pollinations.ai 的圖片 URL
...
要讓人感受到詐騙園區的壓迫感和絕望氛圍。"""
    ...
    async def trigger_random_event(self, member: discord.Member, thread: discord.Thread, 
                                   kkcoin: int, level: int, hp: int, stamina: int, is_first_event: bool = False):
        """觸發隨機事件"""
        try:
            # 根據使用者狀態選擇可能的事件
            ...
            # 執行事件
            await event['handler'](member, thread, kkcoin, level, hp, stamina, is_first_event)

        except Exception as e:
            print(f"執行事件錯誤: {e}")

    def get_possible_events(self, kkcoin: int, level: int, hp: int, stamina: int) -> list:
        """根據使用者狀態獲取可能的事件（以KKCoin回收為主）"""
        events = []
    ...
    async def event_major_confiscation(self, member, thread, kkcoin, level, hp, stamina, is_first):
        """大額沒收（針對富人）"""
        confiscated = min(kkcoin, random.randint(300, 600))
        ...
        content = member.mention if is_first else None
        message = await thread.send(content=content, embed=embed)
        self.event_messages[member.id] = message.id
        
        if is_first:
            await thread.send(f"💡 **提示**: {member.mention} 你可以追蹤此貼文來即時收到園區事件通知！")
        
        self.update_user_kkcoin(member.id, -confiscated)

    async def event_protection_fee(self, member, thread, kkcoin, level, hp, stamina, is_first):
        """保護費"""
        fee = min(kkcoin, random.randint(200, 400))
        ...
        if is_first:
            await thread.send(f"💡 **提示**: {member.mention} 追蹤此貼文可以第一時間知道園區發生什麼事！")
        
        self.update_user_kkcoin(member.id, -fee)

    async def event_beating(self, member, thread, kkcoin, level, hp, stamina, is_first):
        """毆打事件"""
        ...
        if is_first:
            await thread.send(f"💡 {member.mention} 追蹤貼文以接收通知！")
        
        self.update_user_stats(member.id, hp=-hp_loss)

    async def event_police_bribe(self, member, thread, kkcoin, level, hp, stamina, is_first):
        """警方分贓（福利事件）"""
        ...
        if is_first:
            await thread.send(f"💡 {member.mention} 追蹤貼文以獲取更多園區資訊！")
        
        self.update_user_kkcoin(member.id, gain)

    # ==================== 其他事件（簡化版，避免過長）====================
    
    async def event_bribe_demand(self, member, thread, kkcoin, level, hp, stamina, is_first):
        """賄賂勒索"""
        bribe = min(kkcoin, random.randint(150, 350))
    ...
    async def event_gambling_trap(self, member, thread, kkcoin, level, hp, stamina, is_first):
        loss = min(kkcoin, random.randint(100, 300))
    ...
    async def event_fine_penalty(self, member, thread, kkcoin, level, hp, stamina, is_first):
        fine = min(kkcoin, random.randint(80, 200))
    ...
    async def event_equipment_fee(self, member, thread, kkcoin, level, hp, stamina, is_first):
        fee = min(kkcoin, random.randint(50, 150))
    ...
    async def event_kkcoin_confiscation(self, member, thread, kkcoin, level, hp, stamina, is_first):
        confiscated = min(kkcoin, random.randint(100, 250))
    ...
    async def event_forced_overtime(self, member, thread, kkcoin, level, hp, stamina, is_first):
    ...
    async def event_medical_extortion(self, member, thread, kkcoin, level, hp, stamina, is_first):
        cost = min(kkcoin, random.randint(100, 250))
    ...
    async def event_cell_search(self, member, thread, kkcoin, level, hp, stamina, is_first):
    ...
    async def event_blackmail(self, member, thread, kkcoin, level, hp, stamina, is_first):
        amount = min(kkcoin, random.randint(80, 200))
    ...
    async def event_supervisor_inspection(self, member, thread, kkcoin, level, hp, stamina, is_first):
    ...
    async def event_quota_pressure(self, member, thread, kkcoin, level, hp, stamina, is_first):
